...
\textbf{Where the framework supplies a candidate structural condition.} CEMI-style theories identify the candidate integrator (the EM field) but do not specify the structural condition under which the field becomes a stable point of view rather than a transient noise pattern. The framework proposes that the structural condition is $\sigma$-stable closure: an EM field is a candidate carrier of owned experience only if it stabilises a non-trivial $\Sigma_\Ical^{\text{cortex}}$. Fields that fluctuate without closure produce no point of view in this model.
