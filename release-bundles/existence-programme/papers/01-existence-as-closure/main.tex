...
Whether this compression reflects a common structure or coincidence is the empirical question. The framework names the structural condition, derives its consequences, and points to where it could fail. The next sections give that structure.
...
Under (A1, A2) and the cascade chain of Theorem~\ref{thm:cascade-chain}, the $\sigma$-fix dimensions per rung are derived as follows. The cascade-rung-tower demonstration is included here as a standalone reproducibility artifact (\texttt{papers/01-existence-as-closure/repro/rung\_tower\_demo.py}); although the script was originally developed in the withheld Extension IV (\emph{Cosmic Self-Pruning and Frame Recurrence}), no recurrence or cosmic-pruning claims are used in Paper I --- only the per-rung substrate / closure-operator / spectral-$\tau$ construction verified by its demo \textbf{C1}. C1 instantiates $E_8$ (240 root vectors in $\mathbb{R}^8$), $H_4 = V_{600}$ (120 vertices), and $D_4 =$ 24-cell (24 vertices) as substrates, builds their closure operators $\Cph^{(R_k)} = L^{(R_k)} + \varphi^{-2} I$ and spectral $\tau$ involutions, and confirms $\|[\tau, \Cph]\| < 10^{-13}$, $\lambda_{\min}(\Cph) > 0$, and non-trivial $\Sigma$ at each rung. The polytope rungs ($E_8$, $H_4$, $D_4$) are derived from their explicit vertex-graph adjacency spectra; the abstract rungs (40, 16, 8, 0) are bounded by their rank counts.
...
\textbf{CP-rung-9.3 (CLOSED for polytope rungs under spectral $\tau$).} Compute $\sigma\text{-fix}(R_k)$ for $R_k \in \{E_8, 40, D_4, 16, 8, 0\}$. Status: closed for the three polytope rungs ($E_8$, $H_4 = V_{600}$, $D_4$) by demo C1 of \texttt{papers/01-existence-as-closure/repro/rung\_tower\_demo.py}, which instantiates each as a spectral substrate with closure operator $\Cph^{(R_k)} = L^{(R_k)} + \varphi^{-2} I$ and commuting $\tau_{\mathrm{spec}}$, verifies non-trivial $\sigma$-fix dimensions (156, 116, 22 respectively), and confirms $\|[\tau, \Cph]\| < 10^{-13}$ at every rung. Under the icosian $\tau_{\mathrm{ico}}$ convention the corresponding dimensions are 120, 94, 24; the gap is the same basis-change residual flagged in demo D8. Abstract rungs (40, 16, 8, 0) remain rank-upper-bounded; closing these to exact $\sigma$-fix dimensions requires identifying the specific structural representations for those rungs and is open.
...
\item \texttt{papers/01-existence-as-closure/repro/rung\_tower\_demo.py} (C1--C5, 5 demos)
