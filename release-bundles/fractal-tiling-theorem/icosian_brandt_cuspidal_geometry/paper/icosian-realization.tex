...
\newtcolorbox{interpretbox}[1][]{
  ...
  fonttitle=\bfseries, title={Interpretation (non-load-bearing)}, #1}
...
\date{2026-06-02 \quad (pre-peer-review preprint, v1.0.0-rc1)}
...
modular package), the explicit no-fit out-of-sample protocol, and the
honest closure/resonance map of the RH wall (\S\ref{sec:rh}). We claim
\emph{validation and exposition}, not new number theory, and \emph{no}
...
$24$ good primes of norm ${\le}200$. Resolving the two primes over each
split rational prime, the geometry reproduces the \emph{complete}
eigenvalue pair $\{a_\fq,a_{\fq'}\}$, not merely one representative.
...
recover, at every prime, an eigenvalue whose field-theoretic
discriminant is $5\cdot(\text{square})$, i.e.\ an irrational element of
$\Z[\varphi]$; at the inert primes the values match the published
ones exactly.
\item \textbf{The certificate algebra and the RH map
(\S\ref{sec:cert}--\S\ref{sec:rh}).} The Brandt operators are
self-adjoint for the orbit-size measure --- a closure certificate in a
small algebra we make precise. We use the same algebra to locate RH:
the automorphic coefficient side is what the geometry delivers; the
analytic positivity that is RH is the lossless self-adjoint engine,
which we characterize and do not build.
\end{enumerate}
...
$a_\fq=\Nm(\fq)+1-\#E(\OK/\fq)$. No $L$-function table is read. The
Brandt eigenvalues from \eqref{eq:brandt} agree with $a_\fq$ at all
$24$ good primes of norm $\le200$.
...
The curve $31.1\text{-}a1$ has nonsplit multiplicative reduction at
$\fp_{31}$ ($\#E_{\mathrm{ns}}(\F_{31})=q+1$, i.e.\ $a_{\fp_{31}}=-1$).
...
\noindent
(This is the self-adjoint, ``lossless'' end of a broader closure
taxonomy --- dissipative $T^\top GT\prec G$, isometric $T^\top GT=G$ ---
which we do not need here: the Brandt operators are not contractions,
having Eisenstein eigenvalue $\Nm(\fq)+1>1$.)
...
\section{Outlook: a heuristic relation to the Riemann Hypothesis}\label{sec:rh}

This section is explicitly heuristic; nothing in
\S\ref{sec:level31}--\S\ref{sec:cert} depends on it, and it contains no
new theorem. We include it only to state precisely what the verified
structure does \emph{not} reach.
...
\begin{interpretbox}
Informally, one may picture the finite, self-adjoint Brandt operators as
a ``lossless resonator'' and the conjectural absence of off-line zeros
(the GUE statistics of \cite{Montgomery1973,BerryKeating1999}, itself a
conjecture) as the infinite-dimensional analogue. This is a picture for
orientation only. It is not an equivalence, not a theorem, and not used
anywhere above.
\end{interpretbox}
...
\texttt{hecke\_compare.py} & level $31$, $24/24$ out-of-sample ($\Nm\le200$) \\
\texttt{second\_form\_level41.py} & level $41$, $9/9$ out-of-sample vs $E_{41}$ \\
\texttt{genus2\_form\_level61.py} & level $61$ genus-$2$, RM eigenvalues \\
\texttt{multilevel\_dimensions.py} & cuspidal-dimension sequence (Table~\ref{tab:dims}) \\
\texttt{no\_fit\_guard.py} & anti-circularity check \\
\bottomrule
\end{tabular}
\end{center}
...
framed the self-adjoint Hecke operators in a closure/resonance
certificate algebra and used it to state exactly what separates this
verified, finite, coefficient-side structure from the Riemann
Hypothesis: the global analytic positivity, the lossless self-adjoint
engine, which we characterize and do not construct. The value of the
work is a clean, reproducible realization of known arithmetic from
polytope geometry, and an honest account of the gap that remains.
