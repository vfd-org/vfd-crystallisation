...
def prime_ideal_generators(p):
    ...
    inert = (target_norm == p * p)
    # The residue-field roots of x^2 - x - 1 mod p depend only on p, not on the
    # candidate (a, b) -- compute them ONCE (the original recomputed this inside
    # the inner loop, an O(p) cost per grid point).
    roots = [r for r in range(p) if (r * r - r - 1) % p == 0]
    expected = 1 if (inert or not roots) else len(roots)
    # A totally-positive generator of norm N has coordinates of size O(sqrt N),
    # so search a box of side ~3*sqrt(N) rather than the original 4p+6 (which was
    # quadratically oversized for the split case, norm = p).  Grow the box if it
    # somehow comes up short, capped at the original 4p+6 so we can never find
    # FEWER generators than the exhaustive scan did.  The chosen representative
    # may differ from the old one, but a_q is intrinsic to the prime ideal, not
    # the generator -- the <=150 baseline self-check verifies this exactly.
    base = int(target_norm ** 0.5)
    hard_cap = 4 * p + 6
    bound = min(3 * base + 3, hard_cap)
    while True:
        gens = {}
        for a in range(-bound, bound + 1):
            for b in range(-bound, bound + 1):
                ...
                if key not in gens:
                    gens[key] = e
        if len(gens) >= expected or bound >= hard_cap:
            return list(gens.values())
        bound = min(bound * 2, hard_cap)
