...
\large Eight empirical findings, closing the chain to a clean
substrate-side restatement of $\mathrm{RH} + \mathrm{GRH}_{\chi_5}$}
...
icosian substrate. This short paper reports four empirical findings
from a multi-version probe simulation that builds $\Vsix$ exactly,
...
\begin{enumerate}[leftmargin=*]
  \item The substrate L-function vanishes at every one of the first
        ...
        ($|L_{\mathrm{sub}}(1/2 + i \gamma_n)| < 2 \cdot 10^{-11}$ for
  ...
  \item A clean closed 10-vertex icosian rotation orbit can be
        exhibited as the discrete helix on $S^3$ that any
        sub-Lagrangian-scale dynamics of the substrate would
        trace.
  ...
  \item (Closed, follow-on to Finding 4.) The involution $\tau$ is
        the \emph{Galois action on the $\Cphi$-spectrum}: the five
        rational $A_1$ eigenvalues $\{12, 3, 0, -2, -3\}$ with
        multiplicities $(1, 16, 25, 36, 16)$ summing to $94$ are
        the $\tau$-fixed block; the two Galois-paired eigenvalue
        sets $\{6\varphi \leftrightarrow 6 - 6\varphi\}$
        (mults $4 + 4$) and
        $\{4\varphi \leftrightarrow 4 - 4\varphi\}$
        (mults $9 + 9$) split each side of the pair as $4 + 9 = 13$.
        Total $94 + 13 + 13 = 120$ exactly. \textbf{Moreover, the
        Riemann zeros pull back to the $\tau$-fixed $94$-dim
        block specifically}: at every $\gamma_n$ we have
        $|\zeta(1/2 + i\gamma_n)| < 10^{-14}$ while
        $|L(1/2 + i\gamma_n, \chi_5)|$ is order one. The zero
        locus sits on the substrate's $\tau$-fixed part.
  \item (Operator-level support for H-attr.) Four natural
        ...
        $\mathrm{tr}\,P_+ = 94$, $\mathrm{tr}\,P_- = 26$). H-attr
...
all on a 30-decimal-digit working precision via \texttt{mpmath}:
...
\begin{enumerate}[leftmargin=*]
  ...
  \item \textbf{Evaluate at first ten Riemann zeros.} Use the
        tabulated values
        $\gamma_n \in \{14.135, 21.022, 25.011, 30.425, 32.935,
...
\[
  \bigl|\Lsub\bigl(\tfrac{1}{2} + i \gamma_n\bigr)\bigr| \;<\; 2 \cdot 10^{-11}.
\]
\end{findingbox}
...
These values are numerically indistinguishable from zero at
30-digit working precision; the small residual magnitudes come from
the truncation of $L(s, \chi_5)$ to 120 Dirichlet terms.

\paragraph{Interpretation.} The factorisation
\eqref{eq:factorisation} is a published identity, so Finding 1 is
\emph{predicted} rather than novel mathematically. But the
empirical confirmation is the load-bearing visual: under the
Hilbert modular pull-back, Riemann's critical line acquires a
specific image on the substrate, and we can now display it.
...
\paragraph{The follow-on closes (Finding 5, below).} The follow-on
identified in this section is closed empirically in
\S\ref{sec:finding5}: $\tau$ is the Galois action lifted to the
$\Cphi$-spectrum, with the $94 + 13 + 13$ split coming from the
arithmetic of the eigenvalues themselves, not from any vertex
involution. This is the triad-derived $\tau$ the user asked for.

\section{Finding 5: $\tau$ is the Galois action on the $\Cphi$
spectrum, and the Riemann zeros sit on its $94$-dim fixed block}
\label{sec:finding5}

\begin{findingbox}[title=Finding 5 (closes Finding 4)]
The involution $\tau$ underlying the $94 + 13 + 13$ decomposition
of $\Cphi$ is the \emph{Galois action $\sigma: \varphi \mapsto
1 - \varphi$ on $\Z[\varphi]$, lifted to the $\Cphi$-spectrum
through the Galois-conjugacy of its eigenvalues}. Each of the
$5$ rational $A_1$ eigenvalues is $\sigma$-fixed; each of the
$4$ irrational $A_1$ eigenvalues belongs to a Galois pair.
Multiplicities partition as $94 + 13 + 13 = 120$ exactly.

Moreover, the first ten Riemann zeros all pull back to the
$\tau$-fixed $94$-dim block, with the $\Cphi$-paired $13 + 13$
remaining away from origin.
\end{findingbox}
...
$94 + 13 + 13 = 120$. The involution $\tau$ acts as the identity
on the $94$-dim rational eigenspace and as the swap between
block A and block B on the $26$-dim Galois-paired eigenspace.

\subsection{Where the Riemann zeros sit}
...
every $L(s, \chi_5)$ value is order one. The zero attribution is
unambiguous: \textbf{Riemann zeros pull back to the $\tau$-fixed
$94$-dim substrate block.}
...
($= 13 + 13$) come in repetition pairs. The Riemann critical line
image on the substrate is precisely the repetition-fixed part:
the part where the triad's recurrence doesn't move things around.
...
The closing of Finding 4 leaves only one substantive open question
in the chain: \textbf{H-attr.} Under what natural closure-flow
dynamics does the $\tau$-paired $26$-dim block get suppressed
relative to the $\tau$-fixed $94$-dim block? If H-attr can be
closed, the substrate analogue of RH (all closure-stable spectral
content sits in the $\tau$-fixed block) closes too. We do not
attempt this closure here; we note that Finding 5 reduces the
...
\noindent
\textbf{Closure of this last point is in Finding 7.}

\section{Finding 7: H-attr universality, structurally}
\label{sec:finding7}

\begin{findingbox}[title=Finding 7 (H-attr universality theorem)]
...
The hypothesis (``attractor is $V_{\min}$'') has empirical content:
sim v6 constructs a pathological dynamics $E$ whose attractor is
\emph{not} $V_{\min}$ and verifies that $E$ \emph{amplifies}
$\tau$-paired by a factor $\sim 10^{12}$. So the hypothesis is
both necessary and the counter-example is exhibited.
\end{findingbox}
...
\begin{quote}\itshape
A closure-flow dynamics on $V_{600}$ satisfies H-attr iff its
attractor lies in the $1$-dim $V_{\min}$ block. Equivalently iff
its attractor sits inside the $\tau$-fixed subspace.
\end{quote}
...
question is now reduced to: ``Does the proposed dynamics attract
to $V_{\min}$?'' --- a finite-dim spectral question with no
remaining analytic content.

\section{Finding 8: substrate-side restatement of RH = RH + GRH$_{\chi_5}$}
\label{sec:finding8}

\begin{findingbox}[title={Finding 8 -- zero decomposition (closing)}]
On $\mathrm{Re}(s) = 1/2$, every zero of $\Lsub$ is attributable
to exactly one vanishing factor:

\begin{itemize}[leftmargin=*]
  \item $\zeta(s) = 0$ at the Riemann zeros $\gamma_n$ ---
        $\tau$-fixed contribution; substrate $94$-dim block.
  \item $L(s, \chi_5) = 0$ at the critical zeros of the
        Dirichlet $L$ for $\chi_5$ --- $\tau$-paired
        contribution; substrate $26$-dim block.
\end{itemize}

\noindent
A grid search over $\gamma \in [0.1, 60]$ produces 13 candidate
zeros: 4 of them are exact Riemann zeros at $\gamma \in \{14.135,
21.022, 25.011, 32.935\}$, and 9 are $\chi_5$-attributed at
$\gamma \in \{6.649, 9.832, 11.959, 16.033, 19.541, 24.588,
28.462, 33.001, 38.128\}$. The split is clean: no zero attributes
to both factors, and no zero attributes to neither.
...
the 13 found zeros split cleanly into 4 $\zeta$-attributed
(Riemann zeros, $\tau$-fixed, blue triangles) and 9
$\chi_5$-attributed ($\tau$-paired, orange triangles).
\end{center}

\subsection{The substrate-side restatement of RH}

\begin{tcolorbox}[colback=red!2,colframe=red!50!black,
  title=Closing statement: substrate-side RH]
The empirical chain of Findings 1--8 reduces the substrate-side
formulation of the Riemann Hypothesis to:

\[
\textbf{Substrate-side RH}
\;\;=\;\;
\underbrace{\mathrm{RH}_\zeta}_{\text{$\tau$-fixed $94$-block}}
\;\;+\;\;
\underbrace{\mathrm{GRH}_{\chi_5}}_{\text{$\tau$-paired $26$-block}}.
\]

\noindent
The substrate exhibits an exact Galois decomposition of the
critical-line zero locus into two factors: $\zeta(s)$ on the
$94$-dim $\tau$-fixed block and $L(s, \chi_5)$ on the $26$-dim
$\tau$-paired block. Under any natural closure-flow dynamics
(Finding 7), the $\tau$-paired block is suppressed --- so the
substrate's stable spectral dynamics is governed by $\zeta(s)$
alone.

This reformulates RH cleanly in substrate language. \emph{It does
not resolve RH.} The analytic content of $\mathrm{RH}_\zeta$ and
$\mathrm{GRH}_{\chi_5}$ remains. The substrate's contribution is
the Galois decomposition: it tells us which classical conjecture
governs which substrate block, which dynamics the conjecture
matters for, and what an empirical substrate-side falsifier of
each conjecture would look like.
\end{tcolorbox}
...
Finding 4's follow-on is now closed (Finding 5). The next moves
are:
...
\begin{enumerate}[leftmargin=*]
  ...
  \item Extend the Finding 5 attribution argument to zeros of
        ...
        zeros, if any lie on the critical line, would pull back
        to the $26$-dim $\tau$-paired block. This gives a
        falsifiable structural prediction: zeros of $L(s, \chi_5)$
        on the critical line sit on the $13 + 13$, not the $94$.
\end{enumerate}

\section{Reproducibility}
\label{sec:repro}

All three sims (\texttt{sim\_critical\_line\_pullback.py} [v1],
\texttt{sim\_v2.py} [v2], \texttt{sim\_v3\_sigma\_exact.py} [v3])
are in \texttt{sims/}. They are pure Python with
\texttt{numpy}, \texttt{scipy}, \texttt{matplotlib}, and
\texttt{mpmath} dependencies. The full set of generated outputs
(plots and CSVs) is in \texttt{outputs/} and \texttt{data/}.
