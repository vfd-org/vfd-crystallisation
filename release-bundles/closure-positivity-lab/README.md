...
Paper: **`papers/prime-phenomena-ledger.pdf`** — "The Prime Field Through One
Instrument". Interactive: **`explorables/gap-slider.html`** (derive
Hardy–Littlewood by playing) and **`explorables/chebyshev-race.html`** (the
bias race with the 26,861 crossing), both running on data generated by
`explorables/make_data.py` from the same sieve as the ledger.
