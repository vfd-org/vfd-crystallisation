...
$44$ good prime ideals of norm ${\le}200$ (over $25$ rational-prime
norm buckets): at each split norm the two totally positive generators
resolve the \emph{complete} eigenvalue pair $\{a_\fq,a_{\fq'}\}$, not
merely one representative, and the Brandt value at the level ideal
itself recovers the Steinberg datum $a_{\fp_{31}}=-1$ used by the
Atkin--Lehner check.
...
$44$ good prime ideals of norm $\le200$, both Galois-conjugate ideals
resolved individually at every split norm
(\texttt{data/level31\_per\_ideal.json}); the level ideal itself is
excluded from the good-prime comparison, and its Brandt value $-1$ is
exactly the Steinberg datum consumed by the Atkin--Lehner sign check
below.
