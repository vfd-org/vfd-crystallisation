...
session id: 019dde62-f269-7850-a964-2e00a8d57fa3
--------
user
...
codex
I’ll treat this as a referee audit and first map the paper plus the three source trails: local verification JSON/script, b-anomaly README, and the companion aria-chess preprint. I’m going to keep the review evidence-based with line citations rather than proposing edits speculatively.
exec
...
exec
/bin/bash -lc 'wc -l "papers/aria-closure-kernel/paper/main.tex" "papers/aria-closure-kernel/paper"/sections/*.tex "papers/aria-closure-kernel/paper/references.bib" "papers/aria-closure-kernel/repro/verify_kernel.py" "papers/aria-closure-kernel/repro/results.json" "papers/aria-chess-paper/paper/main.tex" "docs/aria-closure-kernel.md" "/mnt/c/Users/nexus/OneDrive/Documents/My Projects/BANOMALY-001/vfd-b-anomaly/README.md"' in /mnt/c/Users/nexus/OneDrive/Documents/My Projects/vfd-crystalisation-paper
exec
/bin/bash -lc 'nl -ba "papers/aria-closure-kernel/paper/main.tex"' in /mnt/c/Users/nexus/OneDrive/Documents/My Projects/vfd-crystalisation-paper
 succeeded in 294ms:
   ...
   148	(b-anomaly's Mode-B caveat is inherited verbatim) nor that the
   149	aria-chess substrate witness establishes a selection theorem on
   150	the adaptive-closure-transport 4-tuple. The structural scope of
   151	this paper is: \emph{one geometry-fixed operator on one fixed
   152	graph appears as the load-bearing object in two independent
   153	empirical works in two distinct regimes, with no shape retuning
   154	between them}.
   155	\end{abstract}
   156	
   157	% =====================================================================
   158	\input{sections/01_introduction.tex}
   159	\input{sections/02_definition.tex}
   160	\input{sections/03_substrate.tex}
   161	\input{sections/04_spectrum.tex}
   162	\input{sections/05_agreement.tex}
   163	\input{sections/06_passive_witness.tex}
   164	\input{sections/07_active_witness.tex}
   165	\input{sections/08_programme_home.tex}
   166	\input{sections/09_limitations.tex}
   167	\input{sections/10_conclusion.tex}
   168	% =====================================================================
   169	
   170	\section*{Reproducibility}
   171	The complete numerical verification (vertex construction,
   172	short-edge graph build, Laplacian spectrum, operator-norm bound,
   173	discrete-to-continuum correlation across three Laplacian variants)
   174	is reproducible from \texttt{repro/verify\_kernel.py} in this
   175	paper's bundle. No randomness, no fitted parameters: all numbers
   176	in \S\ref{sec:substrate}, \S\ref{sec:spectrum}, and
   177	\S\ref{sec:agreement} are deterministic outputs of the script.
   178	The two empirical witness preprints
   179	(b-anomaly~\citep{SmartBAnomaly2026},
   180	aria-chess~\citep{SmartAriaChess2026}) carry their own
   181	reproducibility artifacts; this paper does not duplicate them.
   182	
   183	\bibliographystyle{plainnat}
   184	\bibliography{references}
   185	
   186	\end{document}
...
papers/aria-chess-paper/paper/sections/10_conclusion.tex
 ...
 succeeded in 378ms:
   186 papers/aria-closure-kernel/paper/main.tex
   ...
   101 papers/aria-closure-kernel/paper/sections/04_spectrum.tex
   131 papers/aria-closure-kernel/paper/sections/05_agreement.tex
   131 papers/aria-closure-kernel/paper/sections/06_passive_witness.tex
   169 papers/aria-closure-kernel/paper/sections/07_active_witness.tex
   135 papers/aria-closure-kernel/paper/sections/08_programme_home.tex
...
codex
The main file already shows the paper is trying to police its own scope in the abstract. Next I’m reading all ten section files with line numbers so I can separate properly supported claims from places where the language drifts beyond the operator-witness framing.
exec
...
/bin/bash -lc 'nl -ba "papers/aria-closure-kernel/paper/sections/10_conclusion.tex"' in /mnt/c/Users/nexus/OneDrive/Documents/My Projects/vfd-crystalisation-paper
 succeeded in 477ms:
    ...
    59	shape used by the $\sigma$-attractor reformulation in the RH preprint
    ...
    83	
    84	\subsection{H$_4$ irrep block decomposition (imported context)}
    85	\label{ssec:irrep}
    86	
    87	The eigenspaces of $\Lop$ partition into H$_4$-proper and
    88	$\sigma$-twin Coxeter exponent classes. For H$_4$ proper the
    89	exponents are $\{1, 11, 19, 29\}$; under the
    90	$\sigma$-automorphism of $\mathbb{Z}[\Ph]$ the exponents become
    91	$\{7, 13, 17, 23\}$. The $\sigma$-orbit projector basis used in the
    92	aria-chess companion's recurrent
    93	layer~\citep{SmartAriaChess2026} realises this block decomposition
    94	at machine precision and provides a spectrally clean
    95	H$_4$-equivariant basis there.
    96	
    97	This subsection is imported context from the aria-chess companion;
    98	the irrep block decomposition is \emph{not} verified by
    99	\texttt{repro/verify\_kernel.py} of this paper and is not used as
   100	a load-bearing fact for any operator-witness claim made here. We
   101	include the labelling for orientation only.
 ...
 succeeded in 517ms:
   ...
   113	  landing.} The b-anomaly's Mode-B analysis shows that a
   114	  free-width Gaussian charm-loop proxy fits the same five datasets
   115	  comparably in $\chi^{2}$ at the cost of one extra shape
 ...
 succeeded in 471ms:
    ...
    77	\end{enumerate}
    78	
    79	The recurrence of an open selection gap across these three frames
    80	is a programme-level reading: the gap may be a single mathematical
    81	problem rather than three independent ones, but the present paper
    82	gives no proof of equivalence. The two empirical witnesses landed
    83	in this paper strengthen external confidence in the \emph{response}
    84	primitive without reducing or addressing the selection gap.
    85	
    86	\subsection{What this paper closes vs leaves open}
    87	
    88	\paragraph{Closes (at the operator-witness level).}
    89	\begin{itemize}\itemsep=2pt
    90	\item The operator $\Cph$ is well-defined and positive definite
    91	  on any $(M, L_M)$ satisfying (H1)--(H3); the operator-norm
    92	  identity $\|\Cph^{-1}\| = \Ph^{2}$ holds whenever
    93	  $\lambda_{\min}(L_M) = 0$ (e.g.\ on a connected finite graph
    94	  with the standard combinatorial Laplacian). On substrates where
    95	  $\lambda_{\min}(L_M) > 0$ (e.g.\ Dirichlet-boundary continuum
    96	  cases) the bound $\|\Cph^{-1}\| \leq \Ph^{2}$ holds and is
    97	  generally strict (\S\ref{sec:definition}).
    98	\item The 600-cell instance $\Rsixhundred$ has the construction
    99	  described (\S\ref{sec:substrate}) and the Laplacian spectrum of
   100	  Table~\ref{tab:spectrum}, both reproduced numerically
   101	  (\texttt{repro/verify\_kernel.py}).
   102	\item Discrete-to-continuum agreement at per-vertex Pearson
   103	  correlation $0.976$ on the unweighted variant, with the unweighted
   104	  variant winning the geometry-only criterion against two
   105	  $\Ph$-cocycle weighted controls (\S\ref{sec:agreement}).
   106	\item Same fixed $\Cph$ on same fixed $\Rsixhundred$ appears as
   107	  the load-bearing object in two independent empirical works in
   108	  qualitatively distinct regimes (\S\ref{sec:passive_witness},
   109	  \S\ref{sec:active_witness}).
   110	\end{itemize}
   111	
   112	\paragraph{Leaves open.}
   113	\begin{itemize}\itemsep=2pt
   114	\item \emph{First-principles derivation of $\Ph^{-2}$.} Reported
   115	  as a design-level shift; not derived from a closure functional
   116	  or symmetry argument.
   117	\item \emph{Substrate-uniqueness ablation.} The 600-cell choice is
   118	  post-hoc motivated by the empirical landings; alternative regular
   119	  4-polytopes are an explicit ablation build, not a discharged
   120	  comparison.
   121	\item \emph{Kernel-uniqueness on either empirical landing.} The
   122	  b-anomaly Mode-B caveat (free-width Gaussian alternatives fit
   123	  comparably) and the AIC tie ($w_{\mathrm{VFD}}=0.348$ vs
   124	  $w_{\mathrm{FREE\_C9}}=0.652$) are inherited verbatim from
   125	  ~\citep{SmartBAnomaly2026}.
   126	\item \emph{Selection theorem on ACT.} Lyapunov $V(W)$, edge-space
   127	  decomposition under $2I$-equivariance, full reduced-flow
   128	  convergence — all explicitly not delivered
   129	  in~\citep{SmartAdaptiveClosureTransport2026} and not delivered
   130	  here.
   131	\item \emph{Family-membership theorem.} The programme-home
   132	  positioning of cascade Lyapunov functionals as members of the
   133	  same polynomial-in-$L$ family is reported as
   134	  \emph{programme-positioned}, not formally classified.
   135	\end{itemize}
 ...
 succeeded in 513ms:
   ...
   123	empirical landing — the b-anomaly paper documents Mode-B
   124	free-width Gaussian alternatives that fit comparably in $\chi^{2}$
   125	at the cost of one extra shape parameter, and the aria-chess
   126	preprint does not run a substrate ablation; both caveats are
   127	inherited verbatim. The agreement also does not establish that
 ...
 succeeded in 613ms:
   ...
   127	side (the AIC tie and Mode-B caveats prevent that). It does
   128	establish, at the operator level, that the same fixed $\Cph$ on
   129	the same fixed $\Rsixhundred$ is consistent with one of two
   130	independent empirical landings without parameter retuning. The
   131	second landing is in \S\ref{sec:active_witness}.
 ...
 succeeded in 565ms:
    ...
    55	either empirical landing; the b-anomaly's Mode-B Gaussian
...
codex
The sections are internally cautious overall, but I’ve already seen several phrases that need tightening: “theorem-level rigorous,” “decisive,” and some inherited aria-chess/b-anomaly claims that must be checked against their sources before accepting them. I’m now reading the JSON/script and the two source preprints/documents for numeric provenance.
exec
...
exec
/bin/bash -lc "rg -n \"(18/18|17/18|six|6/6|0\\.976|0\\.888|0\\.884|0\\.923|0\\.997|13\\.555|AIC|Mode-B|Mode B|Gaussian|600-cell|unique|derive|selection|Lyapunov|2I|equivariance|bounded_topk|eta|η|topk|K_7|wake|WAKE|propofol|Propofol|HCP|79\\.78|11\\.58|40\\.6|87\\.5|93\\.8|65\\.4|70\\.6)\" \"papers/aria-closure-kernel/paper\" \"papers/aria-chess-paper/paper\" \"/mnt/c/Users/nexus/OneDrive/Documents/My Projects/BANOMALY-001/vfd-b-anomaly/README.md\"" in /mnt/c/Users/nexus/OneDrive/Documents/My Projects/vfd-crystalisation-paper
exec
/bin/bash -lc "rg -n \"(\\\\Cph|\\\\Ph|\\\\Lop|\\\\Rsixhundred|C_\\\\Ph|C_\\{\\\\Ph\\}|L_\\{V_\\{600\\}\\}|V_\\{600\\}|varphi|\\\\varphi|φ|Phi|Phi\\"'^|'"\\\\Phi|600-cell|600 cell)\" \"papers/aria-closure-kernel/paper\"" in /mnt/c/Users/nexus/OneDrive/Documents/My Projects/vfd-crystalisation-paper
 succeeded in 283ms:
     1	% =====================================================================
     2	\section{Conclusion}\label{sec:conclusion}
     3	% =====================================================================
...
/mnt/c/Users/nexus/OneDrive/Documents/My Projects/BANOMALY-001/vfd-b-anomaly/README.md:58:├── CITATION.cff                   # citation metadata
...
papers/aria-closure-kernel/paper/main.tex:61:neuroscience correspondences plus six drug/sleep EEG
...
papers/aria-closure-kernel/paper/main.tex:110:\noindent\emph{(ii) The 600-cell instance.}
...
papers/aria-closure-kernel/paper/main.tex:137:$\eta\in\{0,0.05,0.20\}$ and one substrate-pinned nonlinearity
papers/aria-closure-kernel/paper/main.tex:138:$\mathrm{bounded\_topk}(\cdot, k\!=\!12)$ at the graph's average
papers/aria-closure-kernel/paper/main.tex:140:2026-04-18) and six drug/sleep EEG signatures.
...
papers/aria-closure-kernel/paper/main.tex:148:(b-anomaly's Mode-B caveat is inherited verbatim) nor that the
papers/aria-closure-kernel/paper/main.tex:149:aria-chess substrate witness establishes a selection theorem on
...
papers/aria-closure-kernel/paper/references.bib:36:% Companion VFD preprints (the two empirical witnesses + selection-layer paper).
papers/aria-closure-kernel/paper/references.bib:40:  title  = {A geometry-derived response kernel for the $B \to K^*\mu^+\mu^-$
papers/aria-closure-kernel/paper/references.bib:49:            eighteen preregistered correspondences and six drug/sleep EEG
papers/aria-closure-kernel/paper/references.bib:50:            signatures from the 600-cell under H$_4$ Coxeter symmetry},
papers/aria-closure-kernel/paper/references.bib:59:            selection},
papers/aria-closure-kernel/paper/references.bib:76:  title  = {The cascade gap operator on the 600-cell substrate},
...
papers/aria-closure-kernel/paper/sections/03_substrate.tex:68:  classical 600-cell connectivity result is well known
...
papers/aria-closure-kernel/paper/sections/07_active_witness.tex:18:  WAKE/RECOVERY) that controls the strength of the recurrent
...
papers/aria-closure-kernel/paper/sections/07_active_witness.tex:84:  WAKE stimulus protocol, the RECOVERY trajectory is bit-identical
papers/aria-closure-kernel/paper/sections/07_active_witness.tex:85:  to the WAKE trajectory.
...
papers/aria-closure-kernel/paper/sections/07_active_witness.tex:110:  deviations below the HCP biological cohort.
...
papers/aria-closure-kernel/paper/sections/07_active_witness.tex:154:Other free params  & 1 amplitude $A$ per dataset        & 1 coupling $\eta$ per condition; v4 stimulus protocols \\
...
papers/aria-closure-kernel/paper/sections/01_introduction.tex:9:and (ii) eighteen preregistered cortical correspondences plus six
...
papers/aria-closure-kernel/paper/sections/01_introduction.tex:95:  six drug/sleep EEG signatures.
...
papers/aria-closure-kernel/paper/sections/01_introduction.tex:113:  landing.} The b-anomaly's Mode-B analysis shows that a
papers/aria-closure-kernel/paper/sections/01_introduction.tex:114:  free-width Gaussian charm-loop proxy fits the same five datasets
...
papers/aria-closure-kernel/paper/sections/01_introduction.tex:153:level (sign uniformity in $5/5$ datasets, an $18/18$ preregistered
...
papers/aria-closure-kernel/paper/sections/01_introduction.tex:166:structural fit; aria-chess $18/18$ + $6/6$ preregistered substrate
...
papers/aria-closure-kernel/paper/sections/08_programme_home.tex:43:\subsection{Response vs selection}
...
papers/aria-closure-kernel/paper/sections/08_programme_home.tex:59:The selection layer is open. It appears as an open hypothesis in
...
papers/aria-closure-kernel/paper/sections/08_programme_home.tex:76:  selection-theorem witness.
papers/aria-closure-kernel/paper/sections/08_programme_home.tex:79:The recurrence of an open selection gap across these three frames
papers/aria-closure-kernel/paper/sections/08_programme_home.tex:84:primitive without reducing or addressing the selection gap.
papers/aria-closure-kernel/paper/sections/08_programme_home.tex:98:\item The 600-cell instance $\Rsixhundred$ has the construction
papers/aria-closure-kernel/paper/sections/08_programme_home.tex:103:  correlation $0.976$ on the unweighted variant, with the unweighted
papers/aria-closure-kernel/paper/sections/08_programme_home.tex:106:\item Same fixed $\Cph$ on same fixed $\Rsixhundred$ appears as
papers/aria-closure-kernel/paper/sections/08_programme_home.tex:115:  as a design-level shift; not derived from a closure functional
papers/aria-closure-kernel/paper/sections/08_programme_home.tex:117:\item \emph{Substrate-uniqueness ablation.} The 600-cell choice is
papers/aria-closure-kernel/paper/sections/08_programme_home.tex:121:\item \emph{Kernel-uniqueness on either empirical landing.} The
papers/aria-closure-kernel/paper/sections/08_programme_home.tex:122:  b-anomaly Mode-B caveat (free-width Gaussian alternatives fit
papers/aria-closure-kernel/paper/sections/08_programme_home.tex:123:  comparably) and the AIC tie ($w_{\mathrm{VFD}}=0.348$ vs
papers/aria-closure-kernel/paper/sections/08_programme_home.tex:126:\item \emph{Selection theorem on ACT.} Lyapunov $V(W)$, edge-space
papers/aria-closure-kernel/paper/sections/08_programme_home.tex:127:  decomposition under $2I$-equivariance, full reduced-flow
papers/aria-closure-kernel/paper/sections/08_programme_home.tex:132:  positioning of cascade Lyapunov functionals as members of the
...
papers/aria-closure-kernel/paper/sections/05_agreement.tex:123:empirical landing — the b-anomaly paper documents Mode-B
papers/aria-closure-kernel/paper/sections/05_agreement.tex:124:free-width Gaussian alternatives that fit comparably in $\chi^{2}$
...
papers/aria-closure-kernel/paper/sections/09_limitations.tex:118:cross-cohort EEG (TUH, NHM) and cross-parcellation HCP
...
papers/aria-closure-kernel/paper/sections/10_conclusion.tex:36:  $\eta\in\{0,0.05,0.20\}$, one substrate-pinned nonlinearity
papers/aria-closure-kernel/paper/sections/10_conclusion.tex:37:  $\mathrm{bounded\_topk}(\cdot, k=12)$ at the graph's average
papers/aria-closure-kernel/paper/sections/10_conclusion.tex:39:  (frozen 2026-04-18) and six drug/sleep EEG signatures.
...
papers/aria-closure-kernel/paper/sections/10_conclusion.tex:55:either empirical landing; the b-anomaly's Mode-B Gaussian
...
papers/aria-closure-kernel/paper/sections/04_spectrum.tex:79:(predicted) — match to six decimal places. The largest eigenvalue
...
papers/aria-closure-kernel/paper/sections/06_passive_witness.tex:79:  (correlation $0.997$ with the continuum kernel, b-anomaly
papers/aria-closure-kernel/paper/sections/06_passive_witness.tex:81:  ($\chi^{2}=13.555$). The two criteria agree on the variant
...
papers/aria-closure-kernel/paper/sections/06_passive_witness.tex:127:side (the AIC tie and Mode-B caveats prevent that). It does
papers/aria-closure-kernel/paper/sections/06_passive_witness.tex:129:the same fixed $\Rsixhundred$ is consistent with one of two
...
papers/aria-chess-paper/paper/main.tex:90:OpenNeuro \texttt{ds005620} point-estimate window for propofol
...
papers/aria-chess-paper/paper/main.tex:185:OpenNeuro propofol cohort \texttt{ds005620}~\citep{OpenNeuroDS005620},
papers/aria-chess-paper/paper/main.tex:188:HCP Consortium~\citep{VanEssen2013HCP} for releasing the public
papers/aria-chess-paper/paper/main.tex:192:the propofol microstate comparison uses Brodbeck et
...
papers/aria-chess-paper/paper/references.bib:91:  title   = {{EEG} microstates of wakefulness and {NREM} sleep},
papers/aria-chess-paper/paper/references.bib:100:@article{VanEssen2013HCP,
papers/aria-chess-paper/paper/references.bib:131:  title        = {{OpenNeuro Dataset ds005620}: propofol-induced loss of consciousness {EEG}},
...
papers/aria-chess-paper/paper/sections/05_results.tex:80:propofol switching, literature-direction predictions for $\Phi$
...
papers/aria-chess-paper/paper/sections/05_results.tex:123:P16 & Conv null mapping                      & $\geq 50\%$      & $70.6\%$ & $\checkmark$ \\
...
papers/aria-chess-paper/paper/sections/05_results.tex:195:WAKE $\cap$ cortical-avalanche band) are non-empty across three
...
papers/aria-chess-paper/paper/sections/02_method.tex:30:variance ratio, propofol switching ratio, propofol continuity drop,
...
papers/aria-chess-paper/paper/sections/02_method.tex:97:empirical reference for the propofol switching ratio
...
papers/aria-chess-paper/paper/sections/02_method.tex:120:microstates. Brodbeck et al.\ is not a propofol-specific paper; we
papers/aria-chess-paper/paper/sections/02_method.tex:122:as a propofol reference. A propofol-specific microstate citation
...
papers/aria-chess-paper/paper/sections/04_consciousness_chain.tex:7:To test high-level signatures — NREM-N3 variance collapse, propofol
papers/aria-chess-paper/paper/sections/04_consciousness_chain.tex:8:regime-switching, propofol $\Phi$ collapse — we add a recurrent
...
papers/aria-chess-paper/paper/sections/04_consciousness_chain.tex:88:machinery~\citep{Tononi2008,BalduzziTononi2008}. The propofol $\Phi$
...
papers/aria-chess-paper/paper/sections/04_consciousness_chain.tex:106:This composite produces the propofol continuity-drop signature
papers/aria-chess-paper/paper/sections/04_consciousness_chain.tex:107:(WAKE composite $0.943$; PROPOFOL composite $0.877$;
...
papers/aria-chess-paper/paper/sections/04_consciousness_chain.tex:146:repeatability under the WAKE stimulus protocol after exposure to
...
papers/aria-chess-paper/paper/sections/07_cross_domain.tex:66:P10 & null perm. mapping (chess, 15 perms)$^{\dagger}$  & $\geq 50\%$ & $65.4\%$ & $\checkmark$ \\
...
papers/aria-chess-paper/paper/sections/07_cross_domain.tex:110:a $65.4$ percentage-point accuracy floor persists under the
...
papers/aria-chess-paper/paper/sections/07_cross_domain.tex:134:P16 & null perm. mapping (15 perms)         & $\geq 50\%$ & $70.6\%$ & $\checkmark$ \\
...
rg: regex parse error:
    (?:(\Cph|\Ph|\Lop|\Rsixhundred|C_\Ph|C_\{\Ph\}|L_\{V_\{600\}\}|V_\{600\}|varphi|\varphi|φ|Phi|Phi\^|\Phi|600-cell|600 cell))
        ^^
error: unrecognized escape sequence

codex
The inherited numbers largely line up: b-anomaly README matches the five-dataset table and AIC caveats; aria-chess matches the 17/18-to-18/18 progression and the EEG/chess/HCP numbers. One inherited phrase still looks too strong in this paper because it converts “six signatures” into “6/6 EEG” and “decisive” language in places where the companion paper itself is more guarded.
exec
/bin/bash -lc "rg -n -F \"\\Cph\" \"papers/aria-closure-kernel/paper\"" in /mnt/c/Users/nexus/OneDrive/Documents/My Projects/vfd-crystalisation-paper
exec
/bin/bash -lc "rg -n -F \"\\Ph\" \"papers/aria-closure-kernel/paper\"" in /mnt/c/Users/nexus/OneDrive/Documents/My Projects/vfd-crystalisation-paper
exec
/bin/bash -lc "rg -n -F \"\\Lop\" \"papers/aria-closure-kernel/paper\"" in /mnt/c/Users/nexus/OneDrive/Documents/My Projects/vfd-crystalisation-paper
exec
/bin/bash -lc "rg -n -F \"\\Rsixhundred\" \"papers/aria-closure-kernel/paper\"" in /mnt/c/Users/nexus/OneDrive/Documents/My Projects/vfd-crystalisation-paper
exec
/bin/bash -lc "rg -n \"\\\\[A-Za-z]+\" \"papers/aria-closure-kernel/paper/main.tex\" \"papers/aria-closure-kernel/paper/sections\" | sed -E 's/.*(\\\\[A-Za-z]+).*/\\1/' | sort | uniq -c | sort -nr" in /mnt/c/Users/nexus/OneDrive/Documents/My Projects/vfd-crystalisation-paper
exec
/bin/bash -lc 'rg -n "(derived|derive|derivation|unique|uniqueness|selection theorem|Lyapunov|2I|equivariance|decisive|establishes|forced|theorem-level|wins|load-bearing|no shape|same fixed|independent|passes|pass)" "papers/aria-closure-kernel/paper/main.tex" "papers/aria-closure-kernel/paper/sections"' in /mnt/c/Users/nexus/OneDrive/Documents/My Projects/vfd-crystalisation-paper
 succeeded in 448ms:
...
papers/aria-closure-kernel/paper/sections/04_spectrum.tex:90:$\sigma$-automorphism of $\mathbb{Z}[\Ph]$ the exponents become
...
papers/aria-closure-kernel/paper/sections/08_programme_home.tex:92:  identity $\|\Cph^{-1}\| = \Ph^{2}$ holds whenever
papers/aria-closure-kernel/paper/sections/08_programme_home.tex:96:  cases) the bound $\|\Cph^{-1}\| \leq \Ph^{2}$ holds and is
papers/aria-closure-kernel/paper/sections/08_programme_home.tex:105:  $\Ph$-cocycle weighted controls (\S\ref{sec:agreement}).
papers/aria-closure-kernel/paper/sections/08_programme_home.tex:114:\item \emph{First-principles derivation of $\Ph^{-2}$.} Reported
...
papers/aria-closure-kernel/paper/sections/08_programme_home.tex:46:geometry: $\Cph$ is fixed by the substrate $\Rsixhundred$ and the
papers/aria-closure-kernel/paper/sections/08_programme_home.tex:90:\item The operator $\Cph$ is well-defined and positive definite
papers/aria-closure-kernel/paper/sections/08_programme_home.tex:92:  identity $\|\Cph^{-1}\| = \Ph^{2}$ holds whenever
papers/aria-closure-kernel/paper/sections/08_programme_home.tex:96:  cases) the bound $\|\Cph^{-1}\| \leq \Ph^{2}$ holds and is
papers/aria-closure-kernel/paper/sections/08_programme_home.tex:106:\item Same fixed $\Cph$ on same fixed $\Rsixhundred$ appears as
papers/aria-closure-kernel/paper/sections/01_introduction.tex:19:\Cph \;=\; L_M + \Ph^{-2} I,
...
papers/aria-closure-kernel/paper/sections/06_passive_witness.tex:128:establish, at the operator level, that the same fixed $\Cph$ on
papers/aria-closure-kernel/paper/sections/05_agreement.tex:6:response $\psi = \Cph^{-1} f$ on $\Rsixhundred$ for a localised
papers/aria-closure-kernel/paper/sections/05_agreement.tex:20:\psi \;=\; \Cph^{-1} f \;=\; (\Lop + \Ph^{-2} I)^{-1} e_{v_{0}}
...
papers/aria-closure-kernel/paper/sections/04_spectrum.tex:68:The smallest eigenvalue of $\Cph = \Lop + \Ph^{-2} I$ is
papers/aria-closure-kernel/paper/sections/04_spectrum.tex:81:$\lambda_{\max}(\Lop) + \Ph^{-2} = (6\Ph + 6) + \Ph^{-2}
papers/aria-closure-kernel/paper/sections/04_spectrum.tex:87:The eigenspaces of $\Lop$ partition into H$_4$-proper and
...
papers/aria-closure-kernel/paper/main.tex:124:Same source vertex, same fixed shift, no parameter fitting.
...
papers/aria-closure-kernel/paper/main.tex:147:unique kernel shape consistent with the b-anomaly data
papers/aria-closure-kernel/paper/main.tex:149:aria-chess substrate witness establishes a selection theorem on
papers/aria-closure-kernel/paper/main.tex:152:graph appears as the load-bearing object in two independent
papers/aria-closure-kernel/paper/main.tex:153:empirical works in two distinct regimes, with no shape retuning
papers/aria-closure-kernel/paper/main.tex:163:\input{sections/06_passive_witness.tex}
papers/aria-closure-kernel/paper/sections/10_conclusion.tex:16:Pearson $\rho = 0.976$ on the unweighted Laplacian, decisively
...
papers/aria-closure-kernel/paper/sections/01_introduction.tex:8:flavour-physics datasets in their passive linear-response regime,
...
papers/aria-closure-kernel/paper/sections/01_introduction.tex:86:  same fixed $\Cph$ on the same $\Rsixhundred$ to describe the
papers/aria-closure-kernel/paper/sections/01_introduction.tex:92:  same fixed $\Cph$ on the same $\Rsixhundred$, augmented by a
papers/aria-closure-kernel/paper/sections/01_introduction.tex:93:  recurrent self-model layer above the substrate, to pass eighteen
papers/aria-closure-kernel/paper/sections/01_introduction.tex:101:\item \emph{Not a derivation of the $\Ph^{-2}$ floor.} The shift
papers/aria-closure-kernel/paper/sections/01_introduction.tex:103:  $\|\Cph^{-1}\|$ at $\Ph^{2}$. It is not derived from a closure
papers/aria-closure-kernel/paper/sections/01_introduction.tex:106:\item \emph{Not a uniqueness claim for $\Rsixhundred$.} Other
papers/aria-closure-kernel/paper/sections/01_introduction.tex:112:\item \emph{Not a kernel-uniqueness claim on either empirical
...
papers/aria-closure-kernel/paper/sections/01_introduction.tex:126:  reduced flow, edge-space decomposition under $2I$-equivariance,
papers/aria-closure-kernel/paper/sections/01_introduction.tex:155:`derives the kernel', `proves uniqueness', or `establishes
papers/aria-closure-kernel/paper/sections/01_introduction.tex:164:discrete-to-continuum correlation; the same fixed operator appearing
...
papers/aria-closure-kernel/paper/sections/06_passive_witness.tex:120:The b-anomaly result is the \emph{passive-regime} empirical
...
papers/aria-closure-kernel/paper/sections/06_passive_witness.tex:128:establish, at the operator level, that the same fixed $\Cph$ on
papers/aria-closure-kernel/paper/sections/06_passive_witness.tex:129:the same fixed $\Rsixhundred$ is consistent with one of two
papers/aria-closure-kernel/paper/sections/06_passive_witness.tex:130:independent empirical landings without parameter retuning. The
papers/aria-closure-kernel/paper/sections/08_programme_home.tex:12:\subsection{Programme home: polynomial-in-$L$ Lyapunov family}
papers/aria-closure-kernel/paper/sections/08_programme_home.tex:55:  time? (Crystallisation / Lyapunov descent dynamics on a
...
papers/aria-closure-kernel/paper/sections/08_programme_home.tex:71:  $2I$-equivariance, and a full reduced-flow convergence theorem
papers/aria-closure-kernel/paper/sections/08_programme_home.tex:75:  a selection theorem; ACT is positioned as the future
papers/aria-closure-kernel/paper/sections/08_programme_home.tex:81:problem rather than three independent ones, but the present paper
papers/aria-closure-kernel/paper/sections/08_programme_home.tex:106:\item Same fixed $\Cph$ on same fixed $\Rsixhundred$ appears as
papers/aria-closure-kernel/paper/sections/08_programme_home.tex:107:  the load-bearing object in two independent empirical works in
papers/aria-closure-kernel/paper/sections/08_programme_home.tex:108:  qualitatively distinct regimes (\S\ref{sec:passive_witness},
papers/aria-closure-kernel/paper/sections/08_programme_home.tex:114:\item \emph{First-principles derivation of $\Ph^{-2}$.} Reported
papers/aria-closure-kernel/paper/sections/08_programme_home.tex:115:  as a design-level shift; not derived from a closure functional
papers/aria-closure-kernel/paper/sections/08_programme_home.tex:117:\item \emph{Substrate-uniqueness ablation.} The 600-cell choice is
papers/aria-closure-kernel/paper/sections/08_programme_home.tex:121:\item \emph{Kernel-uniqueness on either empirical landing.} The
...
papers/aria-closure-kernel/paper/sections/08_programme_home.tex:127:  decomposition under $2I$-equivariance, full reduced-flow
papers/aria-closure-kernel/paper/sections/08_programme_home.tex:132:  positioning of cascade Lyapunov functionals as members of the
papers/aria-closure-kernel/paper/sections/04_spectrum.tex:100:a load-bearing fact for any operator-witness claim made here. We
...
exec
/bin/bash -lc "rg -o \"\\\\citep\\{["'^}]+'"\\}\" \"papers/aria-closure-kernel/paper/main.tex\" \"papers/aria-closure-kernel/paper/sections\" | sort | uniq" in /mnt/c/Users/nexus/OneDrive/Documents/My Projects/vfd-crystalisation-paper
exec
/bin/bash -lc 'rg -n "''^@[a-zA-Z]+'"\\{\" \"papers/aria-closure-kernel/paper/references.bib\"" in /mnt/c/Users/nexus/OneDrive/Documents/My Projects/vfd-crystalisation-paper
exec
/bin/bash -lc "rg -n \"(SmartAriaClosureKernel|citep\\{|ref\\{|label\\{|S\\\\ref|§|—|–|Krüger|σ|φ|ẑ|√|×|≥|≤|±|Δ|Φ|η|κ|ψ|∈|∼|≈|α|χ|C\\"'$'"\\\\times\\"'$P|H'"\\"'$_n'"\\"'$)" "papers/aria-closure-kernel/paper/main.tex" "papers/aria-closure-kernel/paper/sections"' in /mnt/c/Users/nexus/OneDrive/Documents/My Projects/vfd-crystalisation-paper
exec
/bin/bash -lc "rg -n \"(TODO|FIXME|undefined|??|cite\\?|ref\\?|\\\\todo|missing|placeholder|verbatim here|verbatim)\" \"papers/aria-closure-kernel/paper/main.tex\" \"papers/aria-closure-kernel/paper/sections\" \"papers/aria-closure-kernel/paper/references.bib\"" in /mnt/c/Users/nexus/OneDrive/Documents/My Projects/vfd-crystalisation-paper
 succeeded in 250ms:
rg: regex parse error:
    (?:\citep\{[^}]+\})
       ^^
...
94:@article{LHCbAngular2020,
 ...
 exited 2 in 251ms:
rg: regex parse error:
    (?:(TODO|FIXME|undefined|??|cite\?|ref\?|\todo|missing|placeholder|verbatim here|verbatim))
                             ^
...
papers/aria-closure-kernel/paper/main.tex:134:aria-chess~\citep{SmartAriaChess2026}: same $\Cph$ on the same
papers/aria-closure-kernel/paper/main.tex:176:in \S\ref{sec:substrate}, \S\ref{sec:spectrum}, and
papers/aria-closure-kernel/paper/main.tex:177:\S\ref{sec:agreement} are deterministic outputs of the script.
papers/aria-closure-kernel/paper/main.tex:179:(b-anomaly~\citep{SmartBAnomaly2026},
papers/aria-closure-kernel/paper/main.tex:180:aria-chess~\citep{SmartAriaChess2026}) carry their own
...
papers/aria-closure-kernel/paper/sections/06_passive_witness.tex:80:  preprint §3.4) and the LHCb~2025 data $\chi^{2}$
papers/aria-closure-kernel/paper/sections/06_passive_witness.tex:82:  ranking — a two-criterion convergence on the same fixed
papers/aria-closure-kernel/paper/sections/06_passive_witness.tex:131:second landing is in \S\ref{sec:active_witness}.
papers/aria-closure-kernel/paper/sections/08_programme_home.tex:2:\section{Programme home and the open selection layer}\label{sec:programme_home}
...
papers/aria-closure-kernel/paper/sections/02_definition.tex:108:  same constant as the continuum decay scale (Eq.~\eqref{eq:green_continuum}),
papers/aria-closure-kernel/paper/sections/02_definition.tex:110:\item the continuum projection (Eq.~\eqref{eq:green_continuum})
...
papers/aria-closure-kernel/paper/sections/03_substrate.tex:105:The canonical short-edge criterion (Eq.~\eqref{eq:short_edge}) and
...
pole_idx=int(np.argmax(V @ np.array([1.0,0,0,0]))); psi=vk.green_response(C,pole_idx); shells, sizes, _=vk.shell_indices(V,pole_idx)
print('pole_idx', pole_idx)
print('max_within_shell_std', max(float(np.std(psi[idxs])) for idxs in shells.values()))
print('stds', [float(np.std(psi[idxs])) for idxs in shells.values()])
PY" in /mnt/c/Users/nexus/OneDrive/Documents/My Projects/vfd-crystalisation-paper
exec
/bin/bash -lc "rg -n -F \"\\citep{\" \"papers/aria-closure-kernel/paper/main.tex\" \"papers/aria-closure-kernel/paper/sections\"" in /mnt/c/Users/nexus/OneDrive/Documents/My Projects/vfd-crystalisation-paper
exec
/bin/bash -lc 'rg -n -F "??" "papers/aria-closure-kernel/paper/main.tex" "papers/aria-closure-kernel/paper/sections" "papers/aria-closure-kernel/paper/references.bib"' in /mnt/c/Users/nexus/OneDrive/Documents/My Projects/vfd-crystalisation-paper
exec
/bin/bash -lc 'rg -n -F "TODO" "papers/aria-closure-kernel/paper/main.tex" "papers/aria-closure-kernel/paper/sections" "papers/aria-closure-kernel/paper/references.bib"' in /mnt/c/Users/nexus/OneDrive/Documents/My Projects/vfd-crystalisation-paper
 exited 1 in 388ms:
...
papers/aria-closure-kernel/paper/sections/06_passive_witness.tex:71:  $P/S$-basis amplification ($\sim 2.2$~\citep{KrugerMatias2005}).
 ...
 exited 1 in 422ms:
codex
The read-only sandbox blocks shell here-docs, so I’m not rewriting or rerunning the full verifier. I’ve still got the emitted `results.json`, the script source, and the companion sources; for one small symmetry check I’ll use a single `python -c` command that does not write files.
exec
/bin/bash -lc "python3 -c \"import importlib.util,numpy as np; from pathlib import Path; p=Path('papers/aria-closure-kernel/repro/verify_kernel.py'); spec=importlib.util.spec_from_file_location('vk',p); vk=importlib.util.module_from_spec(spec); spec.loader.exec_module(vk); V=vk.build_v600(); A=vk.build_short_edge_graph(V); L,w,U=vk.laplacian_spectrum(A); C=vk.build_C_phi(L); pole_idx=int(np.argmax(V @ np.array([1.0,0,0,0]))); psi=vk.green_response(C,pole_idx); shells,sizes,_=vk.shell_indices(V,pole_idx); print('pole_idx',pole_idx); print('max_within_shell_std', max(float(np.std(psi[idxs])) for idxs in shells.values())); print('stds',[float(np.std(psi[idxs])) for idxs in shells.values()])\"" in /mnt/c/Users/nexus/OneDrive/Documents/My Projects/vfd-crystalisation-paper
 succeeded in 2281ms:
pole_idx 119
max_within_shell_std 1.1152711579477387e-17
stds [0.0, 1.1152711579477387e-17, 5.647850449853784e-18, 5.753426884836168e-18, 3.955778153302621e-18, 5.5763557897386935e-18, 4.17776736080521e-18, 3.3217425607229918e-18, 0.0]
