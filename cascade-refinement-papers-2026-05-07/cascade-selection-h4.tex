...
\item \textbf{Single-state selection.} $\xi^{*}$ is freely chosen
  from $(F^0)^{W(H_4)} \setminus \{0\}$ (a punctured $28$-dimensional fixed subspace);
...
\item \cite{ClosureDynamics} (paper P4): the discrete coboundary
  ...
  finite-dimensional ODE / matrix-exponential argument for flow
...
\cite{HornJohnson} for finite-dimensional spectral / matrix-exponential
...
constants on a connected refined graph (dimension
$\dim_{\R} F^0 = 32$);
...
\begin{itemize}
\item The top-left block $d_n^{*} d_n$ of P4's
  $A_n$, denoted here by $A_n^{\mathrm{vertex}}$ (a local
  shorthand of the present paper, not P4's literal symbol;
  P4 itself names $A_n$ for the full block operator,
...
\item Class (c): midpoint-midpoint diagonal edges
  ...
  face.
\end{itemize}
Old vertices first: every $G_n^{H_4}$-edge $e = \{u, v\}$ is
replaced in $G_{n+1}^{H_4}$ by the length-two path
$u - m_e - v$ via two Class-(a) edges. Hence any $G_n^{H_4}$-path
$u = v_0, v_1, \ldots, v_r = w$ joining old vertices $u, w \in
V(G_n^{H_4})$ is replaced by a length-$2r$ path $v_0, m_{e_1},
v_1, m_{e_2}, \ldots, m_{e_r}, v_r$ in $G_{n+1}^{H_4}$, so
$V(G_n^{H_4}) \subset V(G_{n+1}^{H_4})$ remains mutually
connected by the inductive hypothesis on $G_n^{H_4}$.

New vertices: each level-$(n+1)$ midpoint $m_e$ is adjacent to
both endpoints of its parent edge $e \in E_n^{H_4}$ via Class-(a)
edges, hence to $V(G_n^{H_4})$. Each level-$(n+1)$ centroid
$c_f$ is adjacent to every midpoint $m_{e_i}$ of every edge in
$\partial^\bullet_n(f)$ via Class-(b) edges, hence (through the
already-attached midpoints) to $V(G_n^{H_4})$. Therefore every
vertex of $V(G_{n+1}^{H_4})$ is in the same connected component
as the (mutually connected) old vertex set, which gives connectivity
of $G_{n+1}^{H_4}$. Class-(c) edges are not needed for the
inductive step.
\end{proof}
...
finite-dimensional Hilbert space whose positive spectrum is
nonempty (i.e., $T \neq 0$), write
\[
  \lambda_{+}(T)
...
\subsection{Top-rung analytic preconditions and recorded
...
\begin{proof}
Items (1) and (4) are imported from
\cite{RefinementSpaces}: (1) is
Proposition~\texttt{prop:Xn-Hilbert}, and (4) is
Definition~\texttt{def:p0} together with
Theorem~\texttt{thm:bonding}. Items (2) and (3) are elementary
finite-dimensional consequences of Proposition~\ref{prop:psd}:
the gradient of a quadratic form $\tfrac12 \langle \phi, T\phi
\rangle$ for self-adjoint $T$ is $T\phi$, and the
matrix-exponential semigroup $e^{-tT}$ on a finite-dimensional
Hilbert space exists, is uniformly continuous in $t$, and solves
the Cauchy problem $\dot\Phi = -T\Phi$ uniquely; both are
standard finite-dimensional ODE / matrix-exponential facts
applied to $T = L_N^{\mathrm{block}}$.
\end{proof}
...
of \cite[\S 3.4 lines 609--617]{CascadeMechanism} \emph{in the
witnessed fixed-point sense}, for the chosen $\xi^{*}$ and the
specified initial datum $\phi_N^{(0)} = \phi^{\star}_{N}$ (no
claim is made about basin-of-attraction selection from arbitrary
initial data, beyond the conditional clause (O1.2) below):
...
\end{enumerate}
The relevant top-rung analytic preconditions from $(O0)$ are
verified in Proposition~\ref{prop:o0}; the rung-sequence
formulation of $(O0)$ is not built here (the required inter-rung
bonding map is recorded as available but unused).
\end{theorem}
...
\begin{remark}[Numerical verification (optional)]
\label{rmk:sim}
...
\item Single-state selection of a unique
  ...
  \{0\}$ (a punctured $28$-dimensional fixed subspace); single-ray selection is
  \textbf{(H-uniqueness)} (\S\ref{sec:open-hyp}).
...
branch is satisfied \emph{in the witnessed fixed-point sense} at
every finite top-rung level $N \geq 0$ by the explicit five-tuple
...
consumer API. No basin-of-attraction selection from arbitrary
initial data is claimed.''
...
projection), \texttt{def:An} (P4's full block operator $A_n$,
whose top-left block $d_n^{*} d_n$ we denote here by the local
shorthand $A_n^{\mathrm{vertex}}$),
\texttt{def:Fn} (P4's closure functional), and
\texttt{thm:flow-exists} (style-only: same finite-dimensional ODE /
matrix-exponential argument used in
Proposition~\ref{prop:o0}, item~(O0.3), for
$L_N^{\mathrm{block}}$).
...
finite-dimensional operators: Ch.~VIII.5; bounded operators on Hilbert
spaces: Ch.~II.
