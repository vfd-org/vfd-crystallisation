...
\emph{Schl\"{a}fli-style refinement step}: at each level $n$ it
adds, alongside the new edge-midpoints $M_n^{\bullet}$, a
separate set of face-centroids $C_n^{\bullet}$ (one per face
in the refinement datum $F_n^\bullet$), together with extra
level-$(n+1)$ midpoint-centroid edges and midpoint-midpoint
diagonal edges in a $W^{\bullet}$-equivariant pattern. The chain spaces in the source
...
immediate in the abstract finite-dimensional linear model:
finite-dimensionality and the restriction/averaging pattern are
scalar analogues of \cite{CascadeRefinementSpaces}, and flow
well-posedness is elementary finite-dimensional matrix theory.
The (O3) row and the boundary-energy
...
& Scalar analogues of \cite{CascadeRefinementSpaces} (finite-dimensionality and restriction/averaging pattern); well-posedness of $e^{-t \widetilde A_n}$ is an elementary finite-matrix fact here. Recap in \S\ref{sec:intro}. \\
