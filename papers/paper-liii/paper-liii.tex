...
\paragraph{Update (second pass, June 2026).} A dedicated residual-closure pass (\texttt{docs/residual-closure-derivation.md}, sim suite \texttt{scripts/verify\_residual\_closure.py}, 23/23 PASS) has since upgraded this list:
\begin{itemize}
...
\item \textbf{SM bridge upgraded.} The gauge-group \emph{inventory} $SU(3)\times SU(2)\times U(1)$ is derived as forced structure from the two arenas' automorphism towers ($\mathrm{Der}(\mathbb O) = \mathfrak g_2$, clock-unit stabiliser $= \mathfrak{su}(3)$ identified by computation; internal left $SU(2)$ from the frame split; clock $U(1)$; tower stopped by sedenion zero divisors) --- see \texttt{docs/gauge-group-from-arenas.md}. Embedding pattern, chirality, representations, and couplings remain open.
\end{itemize}
