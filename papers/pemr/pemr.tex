...
\]
agrees with the experimental value $1836.15$ at the $2 \times 10^{-4}$ level with zero fitted continuous parameters. The construction rests on explicit modelling assumptions --- a $\phig$-scaled shell manifold, assignment rules, a shell-dimension model, and a normalisation choice --- all stated and classified by epistemic status. This paper does not claim a first-principles derivation, a Standard Model replacement, or a complete mass theory.
\end{abstract}
...
The present paper explores an analogous algebraic chain in a different physical context. The $E_8$ root system projects onto the vertices of the 600-cell via the standard Coxeter projection~\citep{Coxeter1973}. The 600-cell's Laplacian spectrum contains the eigenvalue $\lambda = 15$ with multiplicity 16, which coincides exactly with the closure invariant $\Delta C = 15$ that governs the leading-order mass ratio. The construction assigns the electron and proton to specific geometric configurations on a nested shell manifold scaled by this polytope's characteristic ratio, then computes their mass ratio from the combinatorial, graph-theoretic, and statistical properties of those configurations. The result is:
\begin{equation}\label{eq:main}
    \frac{m_p}{m_e} = \phig^{1265/81} \approx 1835.8
\end{equation}

The exponent $1265/81$ is a rational number in lowest terms ($\gcd(1265, 81) = 1$), composed of three contributions with explicit origins. No continuous parameters are fitted to the experimental value; the result depends on five discrete structural postulates.

The paper's strongest exact claim is that $\Delta C = 15$ coincides with a 600-cell Laplacian eigenvalue; the remaining fractional correction ($2/3 - 4/81$) is modelled by a low-order spectral moment truncation.
...
Three-term exponent yields $m_p/m_e \approx 1835.8$ & Effective ansatz & Truncation + model step \\
\bottomrule
\end{tabular}
\end{table}
...
\subsection{Mass as spectral gap and the exponential law}\label{sec:masslaw}
...
\textbf{Step 1: Mass as spectral gap.} On any compact geometry, the mass of an excitation is identified with the spectral gap $\lambda_1$ of the Laplacian~\citep{Wilson1974}. This is the standard definition used in lattice gauge theory, condensed matter physics, and is the subject of the Clay Millennium Prize problem on Yang--Mills mass gap~\citep{JaffeWitten2000}. Formally, the heat-kernel trace extracts the spectral gap:
...
\textbf{Step 3: $\phig$-scaling $\to$ $\phig$-eigenvalues (general).} On a domain at scale $\phig^{-k}$, Weyl's asymptotic law~\citep{Weyl1912} gives Laplacian eigenvalues scaling as $\phig^{2k}$ (inversely with the square of the domain size). The mass gap on shell $k$ therefore scales as $\phig^{k}$. Equation~\eqref{eq:spectralgap} confirms this: the spectral gap of the 5-shell path graph is exactly $\phig^{-2}$, a $\phig$-power.
...
\textsc{Epistemic summary:} Steps 1--2 are theorems (spectral gap extraction, path-graph eigenvalue formula). Step 2 proves $\phig$ is native to the spectral gap of the shell graph. Steps 3--4 are standard results (Weyl's law, tensor-product Laplacian additivity). Step 5 applies these to the $\phig$-shell geometry, which is a modelling assumption. Step 6 identifies the correction terms as cumulant coefficients of the heat-kernel expansion (standard spectral theory). The exponential form, the appearance of $\phig$, and the additivity of corrections are all derived; the specific geometry on which they act is postulated.
...
\subsection{Mass law and factorisation}
...
A central question for any $\phig$-scaled framework is: \textit{why $\phig$, and why the 600-cell?} We present a direct spectral connection between the mass law and the polytope.

\subsection{$\Delta C$ coincides with a Laplacian eigenvalue}

The graph Laplacian of the 600-cell vertex adjacency graph (120 vertices, each of degree 12) has a discrete spectrum. Direct diagonalisation of the $120 \times 120$ integer matrix yields the eigenvalue $\lambda = 15$ with multiplicity 16. The closure invariant difference between proton and electron is $\Delta C = 15$. The numerical equality $\Delta C = \lambda$ is exact and independently verifiable.
...
For each admissible support, $\Delta C$ coincides with a 600-cell Laplacian eigenvalue. We interpret this as evidence of a spectral realisation of the closure invariant in the 600-cell geometry. Whether this coincidence is structurally forced (e.g.\ by $H_4$ representation theory) or accidental remains an open question. The computation is exact: the Laplacian is an integer matrix with integer eigenvalues, requiring no numerical precision beyond exact arithmetic.

\textsc{Status: theorem.} The eigenvalues and the closure invariants are both verifiable by direct computation. No fitting or windowing is involved.
...
\begin{itemize}
    \item \textbf{Not a first-principles derivation.} The postulates (shell manifold, assignment rules, factorisation ansatz, normalisation choice) are not derived from accepted fundamental physics. The polytope uniqueness result constrains the scaling constant but does not derive the full construction.
...
The weakest remaining link is the factorisation ansatz (the assumption that the three sectors contribute independently at leading order). The normalisation choice $|V|^2/|E|$ is a residual model selection within a tested family (Section~\ref{sec:robustness}).
