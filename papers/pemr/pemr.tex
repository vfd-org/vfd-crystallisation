...
\midrule
Electron (any single shell) & 0 & Yes ($\lambda_0 = 0$) & 1 & Exact computation \\
Proton $\{2,3,4\}$ & 15 & Yes ($\lambda = 15$) & 16 & Exact computation \\
Support $\{1,2,3,4\}$ & 14 & Yes ($\lambda = 14$) & 36 & Exact computation \\
\bottomrule
\end{tabular}
\end{table}
...
The result falls into \textbf{State B: semi-derived}. This means the mathematical chain from postulates to numerical result is exact and contains no fitted parameters, but the postulates themselves are structural modelling choices rather than consequences of accepted fundamental theory. This is comparable to a lattice model in condensed matter physics: the lattice structure is postulated, but once fixed, the predictions follow without adjustment. In the present case, the four modelling inputs are the $\phig$-scaled shell geometry, the assignment rules, the shell-dimension model, and mass identification with a monotone function of a spectral quantity; the normalisation convention for the second-order correction is a separate stated choice. The mathematical manipulations connecting them to $\phig^{1265/81}$ use only standard spectral and graph-theoretic identities.
...
\textbf{Three-order law} & $\boldsymbol{\phig^{1265/81}}$ & $\boldsymbol{1835.8}$ & $\boldsymbol{2 \times 10^{-4}}$ & \textbf{Selected} \\
