...
The paper also situates the closure framework relative to observational underdetermination in relativistic cosmology, treating closure constraints as a structural selection mechanism on observationally admissible state sets. The paper does not claim equivalence with quantum mechanics, does not introduce Hilbert-space axioms, and does not derive the Schr\"odinger equation. It establishes a stochastic extension of the closure framework that produces probabilistic, spectral, and transition-rate structure from the closure functional alone.
\end{abstract}
...
The present paper addresses these by extending the deterministic dynamics to a stochastic setting. The extension is minimal: a noise term is added to the gradient flow, producing a Langevin equation on $\Sadm$. The resulting stochastic process admits a Fokker--Planck description, a stationary Boltzmann-like distribution, barrier-crossing transition rates, and a path-integral formulation --- all derived from $\Fcal$ and a single noise parameter $\sigma$. A further motivation comes from observational underdetermination in relativistic cosmology: observation alone need not uniquely determine global structure, suggesting the need for a constraint-based selection principle.
...
Candidate physically realised states are then identified with minimisers of $\Fcal$ over $S$:
\begin{equation}
...
\begin{remark}
Manchak's result identifies a structural limitation on observational inference; the closure framework proposes a constraint-based selection mechanism that operates on the resulting admissible set. This does not solve observational underdetermination in full generality, but it clarifies the role that closure constraints play within the present framework.
\end{remark}
...
\begin{enumerate}
    \item \textbf{Stationary distribution:} The process admits the stationary distribution $P_{\mathrm{st}}(\psi) \propto \exp(-2\Fcal(\psi)/\sigma^2)$, unique under standard irreducibility and non-degeneracy conditions for the induced diffusion.
    ...
    \item \textbf{Transition rates:} In the small-noise regime, inter-basin transition rates obey Kramers-type asymptotic scaling $k \asymp \exp(-2\Delta\Fcal/\sigma^2)$.
