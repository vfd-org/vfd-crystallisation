...
\subsection{Constraint-based selection as resolution}

The closure framework resolves this underdetermination by introducing a constraint-first selection principle. Rather than selecting models based on observational equivalence alone, the framework defines physically realised states as those satisfying global closure constraints on the $\phig$-structured geometry.

Let $S = \{M_i \mid M_i \text{ consistent with } \mathrm{Obs}\}$ denote the set of observationally admissible states. Manchak's result implies $|S| \gg 1$. The closure functional introduces a constraint filter:
\begin{equation}
    S_{\mathrm{cl}} = \left\{M \in S \;\middle|\; \Fcal(M) = 0\right\}
\end{equation}
with $|S_{\mathrm{cl}}| \ll |S|$. In the ideal limit, the constraint structure selects a unique or small set of closure-stable configurations from the much larger observationally admissible set.

The physically realised state is then:
\begin{equation}
    M^* = \arg\min_{M \in S}\, \Fcal(M)
\end{equation}

This is the process referred to throughout this programme as \textit{crystallisation}: observation defines a feasible set; constraints define the selection mechanism; physical realisation is constraint satisfaction, not probabilistic collapse.
