...
The closure operator $\Ccal : \mathcal{S}_{\mathrm{adm}} \to \mathcal{S}_{\mathrm{cl}}$ is the metric projection onto the closed subset:
\begin{equation}
    \Ccal(\psi) = \arg\inf_{\phi \in \mathcal{S}_{\mathrm{cl}}} \|\psi - \phi\|
\end{equation}
provided the infimum is attained. If $\mathcal{S}_{\mathrm{cl}}$ is closed and convex (or if $\mathcal{S}_{\mathrm{adm}}$ is finite-dimensional), the projection exists and is unique.
\end{definition}
