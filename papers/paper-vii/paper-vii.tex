...
\begin{equation}
    \Sadm = \left\{\psi \in V \;\middle|\; \|\psi\| = 1,\; E(\psi) < \infty,\; \psi \text{ satisfies boundary conditions}\right\}
\end{equation}
where $E(\psi)$ is an energy functional and the boundary conditions are determined by the physical context. $\Sadm$ is a closed subset of the unit sphere in $V$.
\end{definition}

\begin{remark}
In the finite-dimensional setting relevant to the 600-cell ($V \cong \mathbb{C}^{120}$ or a subspace thereof), $\Sadm$ is compact. This guarantees existence of nearest points and convergence of iterative projections under mild regularity conditions.
\end{remark}
...
A state $\psi$ satisfies the projection constraints if:
\begin{equation}
    \Pcal(\Ccal(\psi)) = \Pcal(\psi)
\end{equation}
where $\Pcal$ is the projection operator from the underlying manifold to observables (Paper~VI~\citep{SmartVI}). This ensures that the closure process does not alter the observable content of the state.
\end{definition}

This constraint connects the closure framework to the gauge-invariance interpretation of Paper~VI: torsional transformations that preserve projection compatibility are gauge symmetries.
...
\textbf{Continuity.} If $\Scl$ is a finite union of smooth submanifolds of $\Sadm$ (as is the case for the discrete shell-support structure), the closure operator is continuous except possibly at equidistant points between distinct components of $\Scl$.
...
If $\Ccal$ is a metric projection onto a closed set, then for any $\psi_0 \in \Sadm$:
\begin{itemize}
    \item $\|\psi_{n+1} - \psi^*\| \leq \|\psi_n - \psi^*\|$ for any $\psi^* \in \Scl$ (non-expansiveness).
    \item If $\psi_0 \in \Scl$, then $\psi_1 = \psi_0$ (idempotence).
    \item The sequence $\{\psi_n\}$ converges to a point in $\Scl$ (in finite dimensions, by compactness).
\end{itemize}
...
This is the operator-level formalisation of the crystallisation process introduced in the bridge paper~\citep{SmartBridge}: the relaxation flow $d\psi/dt = \Pi_{\Sadm}\nabla F(\psi)$ is discretised as the iterative projection~\eqref{eq:iteration}.
...
\begin{equation}\label{eq:functional}
    \Fcal(\psi) = w_1\, d(\psi) + w_2\, \mathrm{Var}(\psi) + w_3\, \Tcal(\psi)
\end{equation}
...
\begin{itemize}
    ...
    \item $\Tcal(\psi) \geq 0$: \textbf{torsional mismatch} --- measures violation of Class~III projection constraints and torsional consistency.
\end{itemize}
...
A state $\psi^* \in \Sadm$ is closure-stable if it is a local minimiser of $\Fcal$:
\begin{equation}
    \psi^* \in \arg\min_{\psi \in \Sadm} \Fcal(\psi)
\end{equation}
subject to the constraint that $\psi^* \in \Scl$ (i.e.\ $\Fcal(\psi^*) = 0$).
\end{definition}

States with $\Fcal(\psi) = 0$ are exactly closed. States with $\Fcal(\psi) > 0$ violate at least one constraint class and carry a nonzero closure residual.
...
Within the framework, distinct attractor basins correspond to distinct stable realised state classes. Candidate particle states are fixed points of $\Ccal$; the basin structure determines which initial configurations resolve to which particle type. The discreteness of the particle spectrum corresponds to the discrete set of attractor basins.
\end{remark}
...
The projection-compatibility condition connects the closure framework to the gauge interpretation of Paper~VI~\citep{SmartVI}:
\begin{equation}\label{eq:projcompat}
    \Pcal(\Ccal(\psi)) = \Pcal(\psi)
\end{equation}

This states that the closure process does not alter the observable content of the state. Torsional transformations $\Tcal_\theta$ that satisfy $\Pcal(\Tcal_\theta\psi) = \Pcal(\psi)$ are gauge symmetries; the closure operator respects this structure by construction (Class~III constraints).
...
The constraint structure is not arbitrary: it follows from the spectral and combinatorial properties of the $\phig$-structured geometry (eigenvalue structure, shell populations, reflection coefficients, Hopf-fiber spectrum). The constraints are \textit{derived from the graph} in the sense that they express specific combinatorial and spectral conditions; they are \textit{not derived from first principles} in the sense that the choice to impose these particular conditions remains a framework-level modelling decision.
