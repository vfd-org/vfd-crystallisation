...
This layered presentation reflects the internal development order of the framework rather than the recommended external reading order. Each layer is obtained from the preceding one within the present framework, conditional on the structural assumptions stated explicitly in Section~\ref{sec:assumptions}. The absence of fitted continuous parameters does not imply absence of structural assumptions; the framework relies on a discrete set of structural choices whose physical justification remains part of the programme (see Section~\ref{sec:assumptions}).

\subsection*{Epistemic status and scope}

This paper documents the internal development of the closure-geometry mass framework. Its role in the paper stack is foundational rather than external-facing: it records the assignment rules, combinatorial invariant, graph construction, and the three-order mass law in the form from which the later bridge paper (Paper~IV) is distilled.

The paper contains both derived results and explicit modelling assumptions. In particular, the $\phig$-structured shell manifold, the assignment rules R1--R5, the shell-support graph, and the leading-order factorisation principle are framework assumptions. Given these, the combinatorial invariant, graph term, degree-variance source, and proton/electron three-order exponent follow within the framework.

This paper does not claim a first-principles derivation from established physics, and it is not intended to serve as the primary external presentation of the mass programme. That role belongs to Paper~IV, which presents the proton/electron result in a more compact and publication-facing form.
...
The framework should therefore be understood as a \textit{candidate geometric substrate} for particle classification and mass, not as an established physical theory. Its value lies in producing a mass law with zero fitted continuous parameters with structural agreement from a small number of explicit assumptions. For a more compact external-facing presentation of the proton/electron result with tighter epistemic framing, see Paper~IV.
...
Combinatorial invariant $C(\Acal)$ & \textsc{framework definition} & Combinatorial class rule \\
...
\begin{itemize}
    \item neutron-proton splitting is of the correct order of magnitude within the existing architecture (not derived)
\end{itemize}
