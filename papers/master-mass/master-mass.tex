...
\title{Particles, Geometry, and Mass from Deterministic Field Closure}
\author{%
  Lee Smart\\[4pt]
...
\begin{abstract}
We develop a structural theory of particles and mass within the deterministic crystallisation framework. Particles are identified with stable closure classes --- equivalence classes of bounded field configurations on a $\phig$-structured manifold, characterised by discrete shell support, winding number, and symmetry sector. Five explicit assignment rules constrain admissible closure classes and uniquely determine the electron as the minimal boundary closure ($\{1\}$) and the proton as the minimal interior composite ($\{2,3,4\}$). A combinatorial class invariant yields $\Delta C = 15$ with zero fitted continuous parameters. Metric-induced class energies show that additive composition is insufficient for mass, requiring a nonlinear law. A closure-graph analysis then yields a three-order mass law,
\[
\frac{m_p}{m_e} = \phig^{\;\Delta C \;+\; \frac{|E|}{|V|} \;-\; \mathrm{Var}(\deg)\,\frac{|E|}{|V|^2}} = \phig^{1265/81},
\]
where $|E|$ and $|V|$ denote the edge and vertex counts of the closure graph. The first-order graph term is derived within the present framework from the per-vertex normalised trace of the closure-graph Laplacian, and the second-order term from degree variance, with a self-consistent normalisation uniquely selected within the tested power-law family. This gives $m_p/m_e \approx 1835.8$, in structural agreement with the observed value $1836.15$ at the $\sim\!2 \times 10^{-4}$ relative level. The agreement is structural rather than metrological: the experimental ratio is known to much higher precision than the framework currently predicts. The neutron-proton mass splitting is compatible with the architecture at the correct scale but not yet derived. Heavier lepton masses (muon, tau) are not reproduced under the current rule set, identifying lepton-generation structure as the primary limitation. The treatment is presented as a falsifiable structural hypothesis and a proof of structural viability for the baryon-lepton mass hierarchy, not as a first-principles derivation or a general mass theory.
\end{abstract}
...
We develop a structural theory in which particles are \textit{closure classes} --- discrete equivalence classes of bounded field configurations on a $\phig$-structured manifold --- and mass emerges from the geometric and graph-theoretic structure of these classes. The development proceeds through six layers:
...
Each layer is derived from the preceding one within the present framework, with zero fitted continuous parameters. The absence of fitted continuous parameters does not imply absence of structural assumptions; the framework relies on a discrete set of structural choices whose physical justification remains part of the programme (see Section~\ref{sec:assumptions}).
...
The closure class is the particle identity. The labels $(S, w, \sigma)$ are the structural analogue of quantum numbers.
...
Seven candidate nonlinear map families were evaluated. Two produce phi-integer-power ratios with zero fitted continuous parameters:
...
The edge-to-vertex ratio $|E|/|V|$ equals $\operatorname{tr}(L)/(2|V|)$, where $L$ is the unnormalised graph Laplacian. This is an exact operator identity.

%==================================================================
\section{First-Order Graph Composition}\label{sec:graphrule}
%==================================================================

The mass of a closure class factorises into independent structural sectors. Since the combinatorial invariant $C$ (depending on shell \textit{indices}) and the graph term $|E|/|V|$ (depending on shell \textit{adjacency}) act on independent structural aspects of the closure class, their contributions multiply at the mass level:
\begin{equation}
...
Within the tested power-law family $N = |V|^a |E|^b$, the normalisation $|V|^2/|E|$ is the unique choice that preserves the self-consistent order-by-order structure and yields sub-$1\%$ agreement. It equals $|V|/g_1$, where $g_1 = |E|/|V|$ is the first-order graph term. This is the \textit{self-consistent normalisation}: each order of the expansion is normalised relative to the previous order.
...
\begin{tabular}{@{}llccl@{}}
\toprule
Order & Term & Exponent & Predicted $m_p/m_e$ & Error \\
\midrule
...
The three-order law arises from a single structural principle: \textit{mass factorises over independent closure-class sectors}.
...
\begin{table}[ht]
\centering
\caption{Result status map.}
\label{tab:status}
\begin{tabular}{@{}llll@{}}
\toprule
Result & Type & Parameters & Status \\
\midrule
Closure-class definition & Structural & 0 & Established \\
Assignment rules R1--R5 & Structural & 0 & Established \\
Unique minimality of $\{1\}$ and $\{2,3,4\}$ & Derived within framework & 0 & Established \\
$\Delta C = 15$ & Derived & 0 & Established \\
Additive energy insufficient & Diagnostic & 0 & Established \\
Bounds $\phig^{15}$--$\phig^{16}$ & Derived within framework & 0 & Established \\
Graph Laplacian term $\operatorname{tr}(L)/(2|V|)$ & Derived within framework & 0 & Established \\
Degree-variance source & Derived within framework & 0 & Established \\
Normalisation $|V|^2/|E|$ & Structurally selected & 0 & Unique within tested family \\
Three-order exponent $1265/81$ & Derived within framework & 0 & Established within framework \\
Ratio $\approx 1835.8$ & Derived within framework & 0 & $2 \times 10^{-4}$ structural agreement \\
Deeper axiomatic justification & Open & --- & Structurally motivated \\
Neutron-proton splitting & Validation & --- & Compatible (correct scale, not derived) \\
Muon/tau generations & Validation & --- & Not reproduced; rule extension needed \\
\bottomrule
\end{tabular}
\end{table}
...
\subsection{Scope of Current Predictive Reach}
...
\begin{tabular}{@{}llccl@{}}
\toprule
Ratio & Assignment & Predicted & Observed & Verdict \\
\midrule
...
The proton-to-electron ratio is the framework's strongest result. The neutron-proton splitting is compatible but not derived. The muon and tau are not reproduced, representing a genuine limitation that constrains the scope of the current claim. The present result should be interpreted as a proof of structural viability for the baryon-lepton mass hierarchy, not as a general mass theory. Broader validation --- particularly extension to lepton generations --- is required to exclude the possibility of structural coincidence in the proton-to-electron case.
