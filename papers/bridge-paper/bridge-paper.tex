...
\title{From Dirac Solutions to Physical Reality:\\A Crystallisation-Based Selection Architecture}
\author{%
  Lee Smart\\[4pt]
...
We propose \textit{crystallisation} as a deterministic, constraint-based selection architecture acting over a restricted admissible subset of the Dirac solution space. Crystallisation is defined formally as asymptotic convergence, under a projected relaxation flow, toward states that extremise a schematic constraint functional $F(\psi)$. The selected state is determined by the geometry of the constraint landscape, not by stochastic projection.

Within the intended interpretation, candidate stable realised state classes, with particle states as the primary examples, correspond to local extrema of $F$; antiparticles are symmetry-related extrema on conjugate solution branches; and annihilation is provisionally interpreted, in a field-theoretic extension, as a transition toward a more stable admissible configuration. The proposal extends rather than replaces the Dirac equation and standard quantum mechanics. The constraint functional is schematic and not uniquely derived; the paper introduces the \textit{selection architecture}, not a claimed final covariant functional.
\end{abstract}
...
Yet the Dirac equation, like the Schr\"odinger equation before it, does not answer a fundamental question: \textit{given the space of admissible states, what determines which admissible configuration is realised in a concrete physical context?}
...
The present paper proposes a candidate resolution: a deterministic selection architecture, termed \textit{crystallisation}, that acts over a restricted admissible subset of the Dirac solution space. The architecture is defined by a schematic constraint functional whose extrema are interpreted as candidate realised admissible configurations. The proposal extends rather than replaces the Dirac framework; it adds a selection layer to the existing mathematical structure without modifying the equation itself.

The present paper introduces the architecture at a schematic level. A formal observer-slot definition compatible with this selection architecture is given in~\citep{SmartXXIX}; a developed closure-projection operator that instantiates the schematic functional $F$ on a concrete substrate is given in~\citep{SmartXXXIII}; the substrate-level closure functional itself is developed in~\citep{SmartXXXVI}. The present paper is intended to be readable independently of these companion works.
...
The paper works primarily at the level of a \textit{Dirac-inspired selection framework}. Interpretations involving annihilation and particle creation are heuristic extensions to field-theoretic settings; a full QFT formulation is deferred. The constraint functional is schematic and proposed, not uniquely determined. In the present paper, ``selection'' refers primarily to the emergence of definite realised configurations within a measurement-conditioned context; a general ontology of state realisation outside such contexts is left open. The present discussion is anchored to the free Dirac equation as the canonical relativistic fermionic state space, while recognising that physically relevant selection contexts generally involve interactions and environmental coupling. The paper should be read as introducing a \textit{selection architecture} with testable structural implications, not as a finished theory.
...
This assigns a probability distribution over outcomes, but within the orthodox formalism no universally accepted physical selection operator is specified:
\begin{equation}
    O_{\text{select}} : \mathcal{S} \to \psi^* \quad \text{(not provided by the orthodox formalism)}
\end{equation}

\begin{definition}[Selection Problem]
Given the solution space $\mathcal{S}$ of Eq.~\eqref{eq:solspace}, the selection problem asks: what determines the realised admissible configuration $\psi^*$ in a concrete physical context?
\end{definition}
...
The admissible domain $\mathcal{S}_{\mathrm{adm}} \subset \mathcal{S}$ consists of normalised, finite-energy states satisfying the relevant boundary conditions and symmetry constraints of a concrete physical context. In measurement settings, $\mathcal{S}_{\mathrm{adm}}$ is further conditioned by the interaction structure, macroscopic boundary conditions, and symmetry constraints of the experimental arrangement.
\end{definition}
...
The coefficients $a_1, a_2, a_3, a_4 > 0$ are structural parameters. At this stage, $C$, $H$, $R$, $V$ should be read as effective structural descriptors rather than fully specified microscopic observables. Their specific operator definitions and the Lorentz-covariance properties of $F$ are not established in the present paper.
...
The static selection statement $\psi^* \in \arg\sup_{\psi \in \mathcal{S}_{\mathrm{adm}}} F(\psi)$ is shorthand for asymptotic convergence under a projected relaxation flow:
\begin{equation}\label{eq:flow}
    \frac{d\psi}{dt} = \Pi_{\mathcal{S}_{\mathrm{adm}}}\, \nabla_\psi F(\psi)
\end{equation}
where $\Pi_{\mathcal{S}_{\mathrm{adm}}}$ denotes projection onto the tangent space of $\mathcal{S}_{\mathrm{adm}}$ (ensuring the flow remains within the admissible domain). The parameter $t$ is introduced here as an auxiliary convergence parameter of the selection architecture; identification with physical coordinate time is not assumed in the present paper.
...
Within the crystallisation framework, candidate stable realised state classes --- with particle states as the intended primary examples --- correspond to local maxima of $F$ over $\mathcal{S}_{\mathrm{adm}}$. A state $\psi^*$ is a candidate stable configuration if:
\begin{equation}
    \delta^2 F[\eta,\eta]\big|_{\psi^*} < 0 \qquad \text{for all nonzero admissible variations } \eta
\end{equation}

In the intended interpretation, distinct attractor basins of the flow~\eqref{eq:flow} would correspond to distinct stable realised state classes, potentially including particle types. The discreteness of the observed particle spectrum would, in this interpretation, be associated with a discrete set of relevant local maxima.
...
A deeper geometric analysis --- connecting $F$ to specific symmetry structures such as polytope geometry --- is developed in companion work~\citep{SmartIV, SmartV, SmartXXXVI} but is beyond the scope of the present paper.
...
\begin{enumerate}
    \item \textbf{Preferred states.} Physically realised states correspond to extrema of a constraint functional. If effective proxies for $F$ or its components can be operationally characterised, preferred states should exhibit higher coherence measures than generic superpositions.

    \item \textbf{Stability spectra.} Candidate stable realised state classes correspond to local maxima of $F$, with particle states as the intended primary examples. The stability spectrum is linked to the constraint geometry rather than to arbitrary parameter choices.
...
\begin{itemize}
    ...
    \item \textbf{No operational protocol for estimating $F$ is yet given.} The present paper does not supply an experimental procedure for reconstructing the constraint functional or its components from measurement data.
...
The architecture defines a projected relaxation flow~\eqref{eq:flow} on a restricted admissible domain $\mathcal{S}_{\mathrm{adm}}$, driven by a schematic constraint functional $F$. Within the intended interpretation: candidate stable realised state classes correspond to local maxima of $F$; antiparticles are symmetry-related extrema on conjugate solution branches; and annihilation is heuristically interpreted as a transition toward a more stable configuration in a field-theoretic extension.
...
\midrule
Selection principle & Axiomatic collapse & Stochastic collapse & Constraint-based extremisation \\
...
Potential empirical signatures & No (interpretive) & Yes (rate) & Candidate coherence proxies, convergence timescales \\
