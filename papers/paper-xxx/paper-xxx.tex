...
Four candidate weighting routes are developed: basin-volume measure under the crystallisation flow, stability-depth weighting from the closure functional, stationary-measure inheritance from the Paper~XIV equilibrium distribution $\Pst \propto e^{-2\Fcal/\sigma^2}$, and a Gleason-type argument showing that quadratic weighting is the unique compatible measure once the admissible-sector geometry has Hilbert-space structure. The stationary-measure route is the strongest: in the equilibrium regime identified in Paper~XIV, where the Nelson density satisfies $|\Psi|^2 = \rho = \Pst$, the effective Born measure coincides with the stationary measure of the closure dynamics. A two-basin toy model demonstrates the mechanism explicitly.
...
This is the central observation: \textit{the Born measure $|\Psi|^2$ is the stationary measure of the closure dynamics at equilibrium.}
...
\begin{proof}
By assumption 3, $|\Psi|^2 = |\sum_i c_i \phi_i|^2$. By assumption 2, the sectors are well-separated, so cross terms $c_i^* c_j \phi_i^* \phi_j$ integrate to zero over any single sector $\Phi_i$. Therefore $\int_{\Phi_i} |\Psi|^2 = |c_i|^2 \|\phi_i\|^2$. The result follows by normalisation.
\end{proof}
...
\begin{enumerate}
    ...
    \item \textbf{Probability}: stationary measure inherited by admissible sectors yields $P_i = |c_i|^2$.
\end{enumerate}

In the equilibrium closure regime, the Born rule need not be introduced as an independent axiom. It coincides with the stationary measure of the underlying closure process, and it is the unique compatible measure on the Hilbert structure produced by the equilibrium tangent dynamics. The assumptions under which this emerges --- equilibrium, well-separated sectors, sector-supported decomposition --- are stated explicitly, and what remains open is identified.
