...
Four candidate weighting routes are developed: basin-volume measure under the crystallisation flow, stability-depth weighting from the closure functional, stationary-measure inheritance from the Paper~XIV equilibrium distribution $\Pst \propto e^{-2\Fcal/\sigma^2}$, and a Gleason-type argument showing that quadratic weighting is the unique compatible measure once the admissible-sector geometry has Hilbert-space structure. The stationary-measure route is the strongest: at the Paper~XIV equilibrium, the Nelson wavefunction satisfies $|\Psi|^2 = \rho = \Pst$, so the Born measure \textit{is} the stationary measure of the closure dynamics. A two-basin toy model demonstrates the mechanism explicitly.
...
The paper develops four candidate routes toward quadratic weighting, works a toy model explicitly, and identifies the conditions under which the Born rule emerges. It does not claim a complete derivation. It provides a candidate substrate explanation and identifies what remains open.
...
The remainder of this paper develops four candidate routes to $w_i$.
...
\begin{proposition}[Born-like weighting from stationary measure]\label{prop:born}
...
The Born rule is not postulated. It emerges as a consequence of:
\begin{enumerate}
...
\end{enumerate}
The quadratic form is not imposed; it is inherited from the equilibrium structure.
\end{remark}
...
Gleason's theorem assumes Hilbert-space structure and derives the Born rule. The VFD contribution is to explain \textit{why} the relevant space has Hilbert structure: because the equilibrium tangent dynamics is Schr\"odinger-type (Papers~XVIII--XXI). The Gleason argument then completes the chain: closure dynamics $\to$ Schr\"odinger structure $\to$ Hilbert space $\to$ Born rule as the unique compatible measure.
\end{remark}
...
This model is illustrative. It is not derived from the full 600-cell construction, but demonstrates the mechanism by which the stationary measure produces Born-like weights.
...
The Born rule, in this picture, is not an axiom but an emergent law: it is the unique compatible measure on the Hilbert structure produced by the equilibrium tangent dynamics, and it coincides with the stationary measure of the underlying closure process. The assumptions under which this emerges are stated explicitly, and what remains open is identified.

The standard quantum predictions are not modified. What changes is the explanatory status of the Born rule: from primitive postulate to candidate consequence of constraint geometry.
