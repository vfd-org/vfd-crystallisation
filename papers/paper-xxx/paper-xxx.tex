...
\begin{itemize}
    \item Route~C provides the \textit{dynamical} origin: the stationary measure of the closure process is $|\Psi|^2$.
...
Together, they suggest: the closure dynamics produces an effective Hilbert-space structure at equilibrium (Papers~XVIII--XXI), and the Born rule is both the stationary measure of the dynamics (Route~C) and the unique compatible measure on that structure (Route~D).
...
\begin{itemize}
    ...
    \item Four candidate weighting routes are developed, with explicit constructions for each.
