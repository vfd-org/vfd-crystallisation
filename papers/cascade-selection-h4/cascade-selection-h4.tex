...
\begin{abstract}
We discharge the cascade-mechanism consumer-API obligation $(O1)$
(top-rung fixed point and convergence,
...
\cite{ClosureDynamics}'s $A_n$ (which acts non-trivially only on
the $\mathrm{Im}(\OO)$ direction) and from
\cite{RefinementCompat}'s $\widetilde A_n$ (which lives in an
abstract scalar pure-midpoint reduction).

Five clauses constitute the named theorem:
...
kernel projection with uniform exponential bound governed by the
spectral gap $\lambda_{+}(L_N^{\mathrm{block}}) > 0$;
$\dim_{\R} (F^0)^{W(H_4)} = 28$ by irreducibility of the standard
$H_4$-reflection representation; and the resulting tuple
discharges $(O1)$ for any chosen $\xi^{*} \in (F^0)^{W(H_4)}
\setminus \{0\}$, with $(O0)$ verified separately as a standing
precondition.
...
\cite{CascadeMechanism}: we supply a witnessed $(O1)$ discharge
for the cascade-closure consumer API on the $H_4$ branch at every
...
and verify $(O0)$ as a standing precondition. $(O2)$ and $(O3)$
...
Theorem~\ref{thm:main}(4)). For each fixed $\xi^{*}$ we
...
\item \textbf{Single-state selection.} $\xi^{*}$ is freely chosen
  from $(F^0)^{W(H_4)} \setminus \{0\}$ (a $28$-dim cone);
...
\item \textbf{$(O2)$, $(O3)$, or the full $(O0)$--$(O3)$ API.}
  $(O0)$ is verified only as a standing precondition for the
  witnessed $(O1)$ tuple. $(O2)$ as stated remains false
...
\item \textbf{Inverse-limit $(O1)$.} We work strictly at finite
  top-rung level $N$; lifting to $N \to \infty$ requires
  hypotheses $(L1)$ refinement-compatibility of unscaled $A_n$
  and $(L3)$ spectral-gap propagation
...
\item \textbf{Identification with upstream operators.}
  ...
  \cite{ClosureDynamics}'s $A_n$ or
  \cite{RefinementCompat}'s $\widetilde A_n$.
\item \textbf{$W(H_4)$-non-equivariant initial data.}
  Convergence still holds (Theorem~\ref{thm:main}(3)), but the
  limit lies in the full $32$-dim
  $\ker L_N^{\mathrm{block}}$, not in the $28$-dim
  $\Psi_N\bigl((F^0)^{W(H_4)}\bigr)$.
...
operator with the same constant: since $L_N^{\mathrm{block}}$
acts as $L^{\mathrm{cl}}_N$ on each of four orthogonal copies of
$\ell^2(V(G_N^{H_4}); \R)$ inside $X_N^{0, H_4}$, the operator
norm of the block operator equals the operator norm of the
scalar Laplacian
(\cite[Ch.~6.4]{HornJohnson}: norm of a block-diagonal operator
is the maximum of the block norms; here all four blocks coincide).
\end{proof}
...
\[
  \Psi_N \colon F^0 \to \ker L_N^{\mathrm{block}},
  \qquad \Psi_N(\xi)(v) := \xi \quad \text{for all }
  v \in V(G_N^{H_4}),
\]
...
The block-diagonal structure of Definition~\ref{def:block-laplacian}
gives
\[
  \ker L_N^{\mathrm{block}}
  \;=\; \ker L^{\mathrm{cl}}_N \oplus \ker L^{\mathrm{cl}}_N
        \oplus \ker L^{\mathrm{cl}}_N \oplus \ker L^{\mathrm{cl}}_N
  \;\cong\; \R^{4} \otimes_{\R} F^0_{\text{summandwise}}
  \;\cong\; F^0,
\]
where the last isomorphism sends a four-tuple of constants
$(c_{H_4}, c_{D_4}, c_{16}, c_{8})$ in
$V_{H_4}, V_{D_4}, V_{16}, V_8$ respectively to the
$F^0$-element $c_{H_4} \oplus c_{D_4} \oplus c_{16} \oplus c_{8}$.
This isomorphism is exactly $\Psi_N$ (constant-function embedding,
fibrewise).

$\R$-linearity of $\Psi_N$ is by construction; injectivity is
...
For every $\Phi(0) \in X_N^{0, H_4}$, the operator semigroup
$e^{-t L_N^{\mathrm{block}}}$ satisfies
\[
  \lim_{t \to \infty} e^{-t L_N^{\mathrm{block}}} \Phi(0)
  \;=\; P_{\ker L_N^{\mathrm{block}}}\,\Phi(0)
  \quad \text{in } X_N^{0, H_4}\text{-norm,}
\]
with uniform exponential bound
\[
...
\lambda_{+}(L^{\mathrm{cl}}_N) > 0$ (the block extension repeats
each scalar eigenvalue with multiplicity $32$, leaving the
smallest positive eigenvalue unchanged).
\end{theorem}
...
\cite[Ch.~2]{Humphreys} for $H_4$ specifically). An irreducible
representation of a finite group has trivial fixed subspace if
and only if the trivial representation does not appear as a
subrepresentation; for the standard reflection representation of
a finite irreducible Coxeter group, this is the case
(\cite[Ch.~5]{Humphreys}: the reflection representation is
faithful and not the trivial representation, and irreducibility
forbids any non-trivial proper invariant subspace, hence in
particular the fixed subspace, being non-zero).
\end{proof}
...
\begin{proposition}[$(O0)$ verification]
\label{prop:o0}
The cascade-mechanism standing analytic preconditions $(O0)$ of
\cite[\S 3.4 lines 595--608]{CascadeMechanism} hold for the
five-tuple supplied in Theorem~\ref{thm:main} below. Specifically:
...
\item \emph{$C^{1}$ pushforward bonding maps.} The
  $(O1)$-witness construction below uses no inter-rung
  pushforward: clauses (1)--(3) of $(O1)$ are top-rung statements
  about $L_N^{\mathrm{block}}$ on the single space $X_N^{0,
  H_4}$. For completeness, when the cascade-mechanism API
  invokes an inter-rung pushforward
  $\pi^{\sharp}_{k+1, k} \colon \Phi_{k+1} \to \Phi_k$ for
  ...
  $(O1)$ discharge but records its availability.
\end{enumerate}
\end{proposition}
...
\item $\Psi_N^{t} := e^{-t L_N^{\mathrm{block}}}$, the gradient
  flow of $R_N$;
\item $\phi^{\star}_{N} := \Psi_N(\xi^{*})$, the constant
...
\phi_N^{(0)})$ discharges the cascade-mechanism $(O1)$ obligation
of \cite[\S 3.4 lines 609--617]{CascadeMechanism}:
...
\begin{proof}
\emph{(O1.1).} By Theorem~\ref{thm:kernel}, $\Psi_N(\xi^{*}) \in
...
($\xi^{*} \in (F^0)^{W(H_4)}$), so $\Psi_N(\xi^{*})$ is a
...
\item $\xi^{*}$-parametrised constant subspace inside
  ...
  $\dim_{\R} \Psi_0\bigl((F^0)^{W(H_4)}\bigr) = 28$
  (image of injective $\Psi_0$).
\end{itemize}
...
\item \textbf{Single-state selection.} See \S\ref{sec:open-hyp}
  \textbf{(H-uniqueness)} below; reducing the $28$-dim $\xi^{*}$
...
the unique one. Reducing the $28$-dim freedom to a single ray
lies \emph{outside} the cascade-closure consumer API.
\end{remark}

\begin{remark}[\textbf{(H-physical-interpretation)}]
\label{rmk:H-phys}
Whether $\Psi_N\bigl((F^0)^{W(H_4)}\bigr) \subset X_N^{0, H_4}$
...
\item Any theorem about $L_N^{\mathrm{block}}$ as a theorem about
  P4's $A_n$ or compat's $\widetilde A_n$: the operators differ
...
\item Single-state selection of a unique
  ...
  \{0\}$ (a $28$-dim cone); single-ray selection is
  \textbf{(H-uniqueness)} (\S\ref{sec:open-hyp}).
...
\cdot \rangle, e^{-t L_N^{\mathrm{block}}}, \Psi_N(\xi^{*}),
\Psi_N(\xi^{*}))$ for any chosen $\xi^{*} \in (F^0)^{W(H_4)}
\setminus \{0\}$, where $L_N^{\mathrm{block}}$ is the auxiliary
block graph Laplacian and $(F^0)^{W(H_4)}$ has explicit real
dimension $28$. The selected state is unique up to the
$28$-dimensional choice of $\xi^{*}$; reducing to a single ray
is the named open hypothesis \textbf{(H-uniqueness)}, outside
the cascade-closure consumer API.''
