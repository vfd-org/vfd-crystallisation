...
This is the simplest dynamics consistent with the closure structure: it uses only $\Fcal$ and the inner product, introduces no additional parameters, and automatically identifies $\Scl$ as the set of attractors.
...
\begin{itemize}
    ...
    \item \textbf{Massive states are displaced:} states off $\Mcl$ carry mass and are not fixed points of the flow unless they are saddle configurations.
...
\end{equation}
where $\|\dot{\psi}\|^2 = \langle\dot{\psi}, \dot{\psi}\rangle$ is the kinetic term and $\Fcal(\psi)$ plays the role of the potential.
\end{definition}
