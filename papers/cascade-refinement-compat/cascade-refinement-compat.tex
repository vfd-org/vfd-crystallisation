...
The Theorems, Proposition, and Corollary below all hold only
...
forced only after the Schur computation.) We fix $c = 1$,
equivalently $\widetilde A_0 = A_0^{\mathrm{vertex}}$. The main
...
\item Writing $\delta(\psi) \in \R^{E_n}$ for the per-edge midpoint
  ...
  $\ker B_n = 0$. Standard spectral graph theory identifies
  $\ker B_n \subseteq \R^{E_n}$ via the rank formula
  $\mathrm{rank}(B_n) = |V_n| - b(G_n)$, where $b(G_n)$ is the number
  of bipartite connected components of $G_n$ (equivalently, the
  signless Laplacian $Q(G_n) := B_n B_n^{T} = D(G_n) + A(G_n)$ acting
  on $\R^{V_n}$ has $\dim \ker Q(G_n) = b(G_n)$, and
  $\mathrm{rank}(B_n) = \mathrm{rank}(B_n B_n^{T})$). Hence
  $\dim \ker B_n = |E_n| - |V_n| + b(G_n)$, and in particular
  $\ker B_n = 0$ whenever $G_n$ is a (simple) forest. The smallest
  simple-graph counterexample is $G_n = C_4$ (the $4$-cycle), where
  $\dim \ker B_n = 1$ and $\ker D_n$ is strictly larger than
...
formula $\dim \ker B_n = |E_n| - |V_n| + b(G_n)$, where $b(G_n)$
counts bipartite connected components, follows from
$\mathrm{rank}(B_n) = |V_n| - b(G_n)$ via the signless-Laplacian
identity $B_n B_n^{T} = D(G_n) + A(G_n)$ and the standard fact that
$\dim \ker (D + A) = b(G_n)$ on a graph (e.g.\
\cite[Prop.~1.3.10]{BrouwerHaemers}). For $G_n = C_4$ this gives
$\dim \ker B_n = 4 - 4 + 1 = 1$, with explicit generator
...
formula gives $(D_n \phi)(v) = (B_n \delta(\phi))(v) = \deg_{G_n}(v)$,
...
when $\ker B_n = 0$ (e.g.\ when $G_n$ is a forest), and $D_n$ is
non-zero on the level-$n$-supported sector
...
curvature $\widetilde A_n$ via Theorem~\ref{thm:schur} in the
abstract pure-midpoint model, with the P3/P4-tower lift
flagged conditional on \textup{(L1)}--\textup{(L3)} of
\S\ref{sec:scope-and-gap}, and the boundary-energy / Schur
substitute for (O2) discharged in the same scope. The
...
when $G_n$ has non-trivial $\ker B_n$ (e.g.\ $G_n = C_4$), and
...
\S\S\ref{sec:schur}--\ref{sec:worked} are verified to exact rational
