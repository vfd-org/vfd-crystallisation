...
overall normalisation $c \cdot 2^n$) that compensates the
refinement-induced halving in \eqref{eq:cobref} once one passes from
the coboundary level to the curvature ($d^{*} d$) level. We fix the
normalisation $c = 1$, equivalently $\widetilde A_0 = A_0^{\mathrm{vertex}}$. The main
...
\item Writing $\delta(\psi) \in \R^{E_n}$ for the per-edge midpoint
  ...
  $\ker B_n = 0$. The unsigned vertex--edge incidence kernel
  $\ker B_n$ is the space of edge-weightings on which the signless
  Laplacian $B_n B_n^{T} = D(G_n) + A(G_n)$ vanishes; classically,
  $\dim \ker B_n = |E_n| - |V_n| + b(G_n)$ where $b(G_n)$ is the
  number of bipartite connected components of $G_n$, so $\ker B_n = 0$
  whenever $G_n$ is a forest. The smallest counterexample is
  $G_n = C_4$ (the $4$-cycle), where $\dim \ker B_n = 1$ and
  $\ker D_n$ is strictly larger than $\mathrm{range}(H_{n+1})$;
...
the boundary-energy / Schur substitute for (O2) discharged in the
same scope. The cascade-mechanism consumer API itself
