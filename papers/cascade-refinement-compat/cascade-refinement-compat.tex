...
\item (already-proved input) the parent-fibre averaging map
  ...
  \texttt{prop:adjoints}, parts (3)--(4)~(lines~655--719); we
  retain the label here for downstream reference;
...
The scaling factor $2^n$ is the unique exponential factor (up to an
overall normalisation $c$, giving $c \cdot 2^n$) for which
Theorem~\ref{thm:schur} below produces the Schur-halving identity
...
\item Writing $\delta(\psi) \in \R^{E_n}$ for the per-edge midpoint
  ...
  \cite{CascadeClosureDynamics}~Definition lines~366--376, which is a
  different operator). Consequently
