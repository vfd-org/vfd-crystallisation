...
Proposition~\texttt{prop:coboundary-refinement}), but the present
...
also fails: differentiating the semigroup identity at $t = 0$ would
recover the false operator identity. Cascade-mechanism's
...
immediate in the abstract finite-dimensional linear model and
inherits unconditionally to the source tower from
\cite{CascadeRefinementSpaces}; the (O3) row and the boundary-energy
...
& \textbf{(O2) as stated remains false.} Strict operator-level identity $p^{0} \widetilde A_{n+1} = \widetilde A_n p^{0}$ fails (Proposition~\ref{prop:strict-fails}), so the flow-level identity $p^{0} \circ e^{-t \widetilde A_{n+1}} = e^{-t \widetilde A_n} \circ p^{0}$ also fails. The boundary-energy / Schur substitute holds exactly: $\Schur(\widetilde A_{n+1}) = \widetilde A_n$ (Theorem~\ref{thm:schur}). This proves the substitute; it does not discharge (O2) as stated.
