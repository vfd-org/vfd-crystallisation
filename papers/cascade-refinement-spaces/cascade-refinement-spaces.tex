...
tags are distinct elements of $E_{n+1}^\bullet$ (this matters
only on rare configurations where the same unordered pair could
be generated by distinct faces; in $H_4$ and $D_4$ it does not
occur at level $0 \to 1$ but is included for safety).
\begin{itemize}
\item \textbf{Vertices.}
...
$p_{n+1, n}^1$ of Definition~\ref{def:edge-parent} averages only
...
\item (Sectorwise Coxeter-$\sigma$, vertex sector.) On the $D_4$
  ...
  $g|_{\Q(\varphi)/\Q} \circ \sigma_n = \sigma_n \circ g|_{\Q(\varphi)/\Q}$
  holds automatically for all $g \in W(D_4)$ on
  $X_n^{0, D_4}|_{\Q(\varphi)/\Q}$. On the $H_4$ branch, the analogous
  ...
  instance of \cite[Theorem~\texttt{thm:pisigma-functorial}]{SigmaRationality}
...
\cite[Theorem~\texttt{thm:pisigma-functorial}]{SigmaRationality}'s
functoriality theorem and is not asserted here; a non-trivial
$W(H_4)$ reflection has $\Q(\varphi)$-rational matrix entries
outside that theorem's hypothesis on $\Q$-forms).''
...
\texttt{def:icosian-ring} (the icosian ring $\Z[\varphi]^4$ as the
arithmetic carrier of $V_{H_4}$);
\texttt{def:V600} ($V_{600}$ vertex set and coordinates, with
$\Z[\varphi]$-rational entries);
