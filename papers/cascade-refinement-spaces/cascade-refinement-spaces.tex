...
\item On the $D_4$ branch ($\bullet = D_4$, $W^\bullet = W(D_4)$):
  ...
  \]
  Although $R$ has entries in $\tfrac{1}{\sqrt{2}} \Q$, the
  conjugation $R \, g_0 \, R^{-1}$ has entries in
  $\tfrac{1}{2}\Z \subset \Q$ (each entry is a sum of products
  $R_{ik} g_{0, kl} R^{-1}_{lj}$ in which the
  $\tfrac{1}{\sqrt{2}}$ factors combine pairwise to a rational
  $\tfrac{1}{2}$; explicit verification on the eight $D_4$
  reflections is straightforward). In particular,
