...
(3) Edge-parent equivariance: by Definition~\ref{def:refinement}
and the equivariant action on labelled descendants (the
$W^\bullet$-action permutes labelled level-$(n+1)$ edges among
themselves, preserving the class-label and the parent face), so
the edge-parent map respects this permutation.
...
\item $\dim_\R X_n^{0, \bullet} = |V(G_n^\bullet)| \cdot 32$ and
  ...
  $|V(G_n^\bullet)|$ counts elements of the vertex set
  $V_n^\bullet$ and $|E(G_n^\bullet)|$ counts labelled
  unoriented edge elements of $E_n^\bullet$
  (Definition~\ref{def:refinement}; in particular tagged
  duplicates are counted separately), with
  $|E(G_n^\bullet)| = |E^{\rm or}(G_n^\bullet)|/2$.
...
$7$ for $F^1$). The vertex set $V_n^\bullet$, the unoriented edge
set $E_n^\bullet$ (as a labelled multiset, per
Definition~\ref{def:refinement}), and the face set $F_n^\bullet$
are all finite by induction from the finite base data
$\mathcal{R}_0^\bullet$
(Definitions~\ref{def:base-graphs},~\ref{def:refinement-datum}):
each refinement step produces $|M_n^\bullet| = |E_n^\bullet|$
midpoints, $|C_n^\bullet| = |F_n^\bullet|$ centroids, a number of
new edges bounded by $2|E_n^\bullet| + \sum_{f \in F_n^\bullet}
2 \cdot \mathrm{len}(\partial_n^\bullet f)$ (classes
(a)+(b)+(c)), and a number of new faces $\sum_{f \in F_n^\bullet}
\mathrm{len}(\partial_n^\bullet f)$, all finite. Hence
$E^{\rm or}(G_n^\bullet)$ is finite.
A finite direct sum of finite-dimensional Euclidean spaces under
a positive-definite bilinear form is a finite-dimensional Hilbert
...
$f_\infty \in \ell^2(V_\infty^\bullet; F^0)$: by monotone
convergence over the increasing finite exhaustion
$V(G_n^\bullet) \uparrow V_\infty^\bullet$ together with
Lemma~\ref{lem:Xinv-limit-exists},
$\sum_{v \in V_\infty^\bullet} \|f_\infty(v)\|^2
= \lim_n \sum_{v \in V(G_n^\bullet)} \|f_n(v)\|^2
= \lim_n \|f_n\|^2 < \infty$.
...
\emph{Inner product preservation.} By polarization over the real
Hilbert structure (Lemma~\ref{lem:Xinv-limit-exists}) and
absolute convergence of the cross-term series via Cauchy--Schwarz
on $\ell^2(V_\infty^\bullet; F^0)$,
...
\langle \Phi(f), \Phi(g) \rangle_{\ell^2}$, where the second
equality uses monotone convergence on the increasing finite
exhaustion $V(G_n^\bullet) \uparrow V_\infty^\bullet$ for each
of the polarization summands.
...
\item On the $D_4$ branch ($\bullet = D_4$, $W^\bullet = W(D_4)$):
  ...
  standard $D_4$-reflection representation transported from the
  abstract $D_4$ root system through the orthogonal identification
  $R \in \mathrm{O}(4)$ that realizes $V_{24}$ as the rescaled
  $D_4$ root polytope in
  \cite[Definition~\texttt{def:V24} and
  Proposition~\texttt{prop:24-in-600}]{AlgebraicSubstrate}; that is,
  for $g \in W(D_4)$ acting on the abstract root space by an
  even signed-permutation matrix $g_0$ in
  $\mathrm{Mat}_4(\Z)$ (\cite[Ch.~2]{Humphreys}), define
  \[
    \rho_{F^0}^{D_4}(g) := R \, g_0 \, R^{-1} \in \mathrm{O}(V_{D_4}).
  \]
  Writing $R = \tfrac{1}{\sqrt{2}} H$ where $H \in \mathrm{Mat}_4(\Z)$
  is the integer matrix from
  \cite[Proposition~\texttt{prop:24-in-600}]{AlgebraicSubstrate}
  (so $H = \mathrm{diag}\bigl(\begin{smallmatrix} 1 & 1\\ 1 & -1
  \end{smallmatrix}, \begin{smallmatrix} 1 & 1\\ 1 & -1
  \end{smallmatrix}\bigr)$ and $H H^T = 2 I_4$), the conjugation
  is
  \[
    R \, g_0 \, R^{-1}
    = \tfrac{1}{\sqrt{2}} H \cdot g_0 \cdot \sqrt{2} H^{-1}
    = H \, g_0 \, H^{-1}
    = \tfrac{1}{2}\, H \, g_0 \, H^T,
  \]
  using $H^{-1} = \tfrac{1}{2} H^T$ (since $H H^T = 2 I_4$). The
  product $H \, g_0 \, H^T$ has entries in $\Z$, hence
  $\rho_{F^0}^{D_4}(g) \in \tfrac{1}{2}\Z^{\,4 \times 4} \subset
  \Q^{\,4 \times 4}$. In particular,
  $\rho_{F^0}^{D_4}(g)$ preserves the $\Q$-form
  $V_{D_4, \Q} = \Q^4$ for every $g \in W(D_4)$. The
  representation acts trivially on $V_{H_4}, V_{16}, V_8$.
\end{itemize}
...
\item (Sectorwise Coxeter-$\sigma$, vertex sector.)
  ...
  \item ($D_4$ branch.) For $\bullet = D_4$ and any $g \in W(D_4)$,
    $\rho_{F^0}^{D_4}(g)$ has matrix entries in $\Q$ (in the
    ambient $V_{D_4} = \R^4$ basis used here), as established in
    Definition~\ref{def:coxeter}, hence preserves the $\Q$-form
    $V_{D_4, \Q} = \Q^4$. It acts trivially on
    $V_{H_4}, V_{16}, V_8$. Therefore $g|_{\Q(\varphi)/\Q}$ is
...
For downstream papers needing the full $W(H_4)$-$\sigma$
relationship (e.g.\ in spectral-arithmetic constructions),
a natural formulation is as an intertwining with $\sigma$-conjugate
Coxeter elements $g^\sigma$ rather than as commutation:
$\sigma \circ g = g^\sigma \circ \sigma$ where $g^\sigma$ is the
matrix-coefficient $\sigma$-conjugate of $g$.
\end{remark}
