...
\item $V_{16} = \Cl(1, 3)$ with the Hilbert--Schmidt-style trace
  ...
  via \cite[Definition~\texttt{def:Cl-basis}]{AlgebraicSubstrate};
...
\cite[Theorem~\texttt{thm:pisigma-functorial}]{SigmaRationality} is
...
\S\texttt{def:sigmaV}; functoriality of the scalar
extension and Theorem~\texttt{thm:pisigma-functorial} ($\sigma$ commutes
...
$\Z[\varphi]$-rational coordinates); \texttt{def:V600} ($V_{600}$
vertex set and coordinates); \texttt{def:V24} and
\texttt{prop:24-in-600} ($V_{24} \subset V_{600}$ as the rescaled
$D_4$ root polytope); \texttt{def:Cl-basis} (the $\Cl(1, 3)$
monomial basis $\{\gamma_A\}$); \texttt{def:octonions} (standard
octonion basis); sector fibre conventions for $V_{16}$ and $V_8$.
...
$\Q(\varphi)$-rational coordinates of the icosian roots; the
matrix-entry statement follows by the standard reflection formula
and is used in this paper's $H_4$ branch.
