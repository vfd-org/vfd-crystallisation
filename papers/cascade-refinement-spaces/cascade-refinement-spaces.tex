...
\item (Sectorwise Coxeter-$\sigma$, vertex sector.)
  Write $g|_{\Q(\varphi)/\Q}$ for the action of $g \in W^\bullet$
  on the mixed-form vertex space
  $X_n^{0, \bullet}|_{\Q(\varphi)/\Q}$, defined by the same
  formula as in Definition~\ref{def:coxeter} provided the
  fibrewise reflection $\rho_{F^0}^\bullet(g)$ preserves the
  rationality data $F^0_{\Q(\varphi)/\Q}$ (so that the resulting
  function is again $F^0_{\Q(\varphi)/\Q}$-valued).
  On the $D_4$ branch ($\bullet = D_4$, $W^\bullet = W(D_4)$),
  Definition~\ref{def:coxeter} has $W(D_4)$ acting on $V_{D_4}$
  by the standard $D_4$-reflection representation, which has
  matrix entries in $\Q$ (it preserves the $\Q$-form
  $V_{D_4, \Q} = \Q^4$, by the standard root-system structure
  in \cite[Definitions~\texttt{def:V24},
  \texttt{def:Q-forms}]{AlgebraicSubstrate}); it acts trivially
  on $V_{H_4}, V_{16}, V_8$. Therefore $g|_{\Q(\varphi)/\Q}$ is
  well-defined for all $g \in W(D_4)$. Since $\sigma$ acts
  trivially on $V_{D_4, \Q}, V_{16, \Q}, V_{8, \Q}$, the
  commutation
  \[
  ...
  $X_n^{0, D_4}|_{\Q(\varphi)/\Q}$. On the $H_4$ branch, the
  analogous automatic-commutation statement holds for all
  $g \in W(H_4)$ on the $\sigma$-trivial summands
  $V_{D_4, \Q}, V_{16, \Q}, V_{8, \Q}$ (on which $W(H_4)$ acts
  trivially by Definition~\ref{def:coxeter}); the full statement
  on the $V_{H_4}$ summand is not delivered by
  ...
  standard $\Q^4 \subset \Q(\varphi)^4$), and is not claimed
