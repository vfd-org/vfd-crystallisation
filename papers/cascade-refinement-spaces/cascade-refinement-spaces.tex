...
\cite[Prop.~5.3]{AlgebraicSubstrate}.
...
\item \textbf{Edges.} $E_{n+1}^\bullet$ is the disjoint union of
  ...
  \item \emph{Centroid-midpoint reconnection edges:} the empty
    set in this paper. (Some Schl\"{a}fli-style refinement schemes
    in the literature add edges between centroids of adjacent
    faces; we do not include those, and downstream uses of this
    paper do not require them.)
  \end{enumerate}
\item \textbf{Faces.} $F_{n+1}^\bullet$ is determined by the new
  edge set: each level-$n$ face $f$ with edge sequence
  $(e_1, \ldots, e_k)$ produces $k$ level-$(n+1)$ triangle faces
  $(m_{e_i}, c_f, m_{e_{i+1}})$ (cyclic indices), each with edge
  sequence
  $(\{m_{e_i}, c_f\}, \{c_f, m_{e_{i+1}}\}, \{m_{e_{i+1}}, m_{e_i}\})$.
  Note: the third edge of each triangle, $\{m_{e_{i+1}}, m_{e_i}\}$,
  is \emph{not} an element of $E_{n+1}^\bullet$ as constructed
  above; we record this as a defect that the present paper does
  not need (the load-bearing downstream uses of $G_{n+1}^\bullet$
  in this paper, and in
  \cite{CascadeClosureDynamics, CascadeRefinementCompat}, depend
  only on the vertex inclusion and the edge classes (a)+(b), not
  on the level-$(n+1)$ face structure).
\item \textbf{Boundary map.} $\partial_{n+1}^\bullet$ is induced
  on the level-$(n+1)$ triangles by the explicit construction
  above.
\item \textbf{$W^\bullet$-action.} $W^\bullet$ acts on
  ...
  permutes each of the edge classes (a)+(b) among themselves.
\end{itemize}
\end{definition}

\begin{remark}[Scope of the construction]
\label{rmk:refinement-scope}
This paper uses Definition~\ref{def:refinement} only to construct
the chain spaces $X_n^{0, \bullet}$, $X_n^{1, \bullet}$, the
bonding maps, and the intertwining identities below.
The level-$(n+1)$ face structure (and the missing
midpoint-midpoint edges) play no role in those constructions.
Downstream papers needing a fuller cell-complex structure on
$\mathcal{R}_n^\bullet$ may extend Definition~\ref{def:refinement}
...
\cite[Def.~6.2]{AlgebraicSubstrate}, which is a $\Q$-basis of a
...
\cite[Def.~7.1]{AlgebraicSubstrate}). For $V_{H_4} = \R^4$ the
...
\item (Refinement-$\sigma$, vertex sector, mixed-form level.) On
  ...
  Definition~\ref{def:sigma}. Analogously
  $\sigma_n \circ j_{n, n+1}^0|_{\Q(\varphi)/\Q} =
  j_{n, n+1}^0|_{\Q(\varphi)/\Q} \circ \sigma_{n+1}$.
\item (Sectorwise Coxeter-$\sigma$, vertex sector.) On the $D_4$
  ...
  the full statement on the $V_{H_4}$ summand fails
...
\cite[Thm.~4.5]{SigmaRationality} requires a $\Q$-linear
endomorphism $T$ of the $\Q$-vector space $V$ \emph{defined over the
$\sigma$-fixed $\Q$-form}: concretely, $T$ must restrict to
$V \otimes 1 \subset V \otimes_\Q \Q(\varphi)$. On the
$V_{H_4, \Q}$ component, the $W(H_4)$ action is given by reflection
matrices whose coefficients lie in $\Q(\varphi)$ (not in $\Q$),
because the icosian roots have $\Q(\varphi)$-rational coordinates
\cite[\S 3]{AlgebraicSubstrate}. Such a reflection is $\Q(\varphi)$-linear
(hence a fortiori $\Q$-linear on the underlying $\Q$-vector space
$V_{H_4, \Q(\varphi)}$), but it does \emph{not} restrict to a
$\Q$-linear endomorphism of the $\sigma$-fixed subspace
$V_{H_4, \Q} \otimes 1$: a non-trivial $\Q(\varphi)$-coefficient
reflection of a $\Q$-vector carries it out of $V_{H_4, \Q} \otimes 1$
into the general $V_{H_4, \Q(\varphi)}$ space. The hypothesis of
\cite[Thm.~4.5]{SigmaRationality} is therefore not satisfied in
the form required for automatic $\sigma$-commutation, and in
general $g \circ \sigma$ and $\sigma \circ g$ \emph{do not} commute
on the $V_{H_4}$ sector for $g \in W(H_4)$ acting non-trivially on
$V_{H_4}$.
...
functoriality theorem \cite[Thm.~4.5]{SigmaRationality}, the scalar
...
of \cite[Thm.~4.5]{SigmaRationality} is not satisfied on that
summand).''
...
(Paper P1.) Pinned results used in this paper:
Definition~2.5 (coefficientwise $\sigma_V$ on $V \otimes_\Q
\Q(\varphi)$); Definition~4.1 (the projector $\Pi_\sigma$);
Lemma~2.6 ($\sigma$ commutes with $\Q$-linear endomorphisms after
scalar extension); Theorem~4.5 (functoriality of the scalar
extension of any $\Q$-linear map between finite-dimensional
$\Q$-vector spaces).
...
(Paper P2.) Pinned results used in this paper:
\S 3 (icosian model of $V_{H_4}$, $\Q(\varphi)$-rationality of the
$W(H_4)$ matrix entries); \S 5 (the $V_{600}$ and $V_{24}$ vertex
sets, including their explicit coordinates); Proposition~5.3
($V_{24} \subset V_{600}$ as the rescaled $D_4$ root polytope);
Definition~6.2 (the $\Cl(1, 3)$ monomial basis $\{\gamma_A\}$);
Definition~7.1 (the standard octonion basis); \S\S 6, 7 (sector
fibre conventions for $V_{16}$ and $V_8$).
