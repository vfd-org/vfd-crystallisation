...
Connected composite states --- such as the proton on shells $\{2,3,4\}$ --- satisfy all closure constraints and correspond to fixed points of the closure operator. Disconnected states --- such as a quark-like configuration on $\{2,4\}$ --- violate the connectivity constraint and carry a nonzero gap penalty in the variational formulation, placing them outside $\Scl$. Confinement is therefore reinterpreted, within the present framework, as the statement that only connected composites are closure-stable; isolated quark-like states are excluded by the constraint structure itself rather than by an explicit force law at this level of description.
...
In the Standard Model, confinement is a dynamical property of the $\mathrm{SU}(3)$ gauge theory: the strong coupling grows at large distances, preventing colour-charged states from being isolated. This paper offers a complementary structural interpretation: confinement as a consequence of multi-node closure constraints. Within the present framework, states with disconnected shell support are excluded from $\Scl$ by the constraint structure itself rather than by an explicit force law at this level of description.
...
\end{equation}
where each component $\psi_i$ occupies a subset of the shell structure. The composite space is equipped with the product norm $\|\Psi\|^2 = \sum_{i=1}^n \|\psi_i\|^2$, or any equivalent norm on the finite-dimensional product space. The composite shell support is the union:
\begin{equation}
...
The neutron shares the proton's shell support $\{2,3,4\}$ and is closure-stable. It differs from the proton in the symmetry sector (charged vs.\ neutral baryon), corresponding, in the present framework, to a small perturbation in the torsional/symmetry sector. The neutron-proton mass splitting ($\Delta m \approx 1.3$ MeV) is compatible with a small closure-residual difference within the same attractor basin, but is not derived in the present framework.
\end{example}
...
Two-node (connected) & $|S| = 2$, consecutive & Connected & Meson-like composites (within framework) \\
Three-node (connected) & $|S| = 3$, consecutive & Connected & Baryon-like composites (within framework) \\
