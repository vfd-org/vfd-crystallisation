...
X_n^{1, \bullet}$, where $R_n$ is the curvature form, $E_n$ the
indefinite off-diagonal coboundary form, and $Q_n$ the mass form,
built respectively from the discrete graph Laplacian, coboundary,
and mass operator of \S\ref{sec:operators}. The
...
as $\frac{d}{dt} F_n(\Phi_n(t)) = -\|L_n \Phi_n(t)\|^2 \leq 0$
(Proposition~\ref{prop:energy-dissipation}).

A discrete-time Euler scheme $\Phi_{n, k+1} = (I - \eta L_n)
\Phi_{n, k}$ is analysed in
Section~\ref{sec:discrete-time}. One-step energy descent under a
step-size condition is separated cleanly from norm contractivity
...
(Proposition~\ref{prop:mass-refinement}). For arbitrary $(\alpha,
...
An earlier foundations draft contained a broad ``all correspondences
are Borel / equivariant'' framing as a global theorem-grade claim;
this paper deliberately does not rely on that, and instead supplies
an explicit generator-family list of \emph{admissible finite-level
operator expressions} (Definition~\ref{def:admissible}; the legacy
name ``intertwiner'' is retained for continuity but is purely
syntactic, not equivariance-implying), with printed operator-norm
bounds and continuity/Borel measurability following directly from
...
organise the cascade rungs. An earlier draft of the cascade
correspondence foundations manuscript invoked this at an abstract
global level via a broad ``correspondence theorem'' asserting that
every correspondence arising from closure dynamics is Borel and
symmetry-equivariant. Internal review identified that claim as
false in the form stated, and recommended replacement by
case-by-case operator-level bounds.
...
and derive its dynamics rigorously. Every operator in the
generator-family list of \S\ref{sec:intertwiners} (bonding maps,
flow-operator family, Coxeter actions, sector projections,
coboundaries, adjoints, Laplacian and mass blocks, $G_2$-stabilizer
fibre map) is given an explicit norm bound; downstream papers
P5--P13 may freely compose these into admissible finite-level
expressions, with bounds inherited via
...
\item Refinement compatibility for the mass-only case
  ...
  (Proposition~\ref{prop:mass-refinement}). For arbitrary
...
uniqueness claim, any prior global Borel/equivariance correspondence
theorem from earlier foundations drafts, or any continuum-limit
cascade note.
...
scheme and separates one-step energy descent (under a step-size
condition) from norm contractivity.
...
$(d_n f)(-e) = \pi_{F^0 \to F^1}\bigl(
f(\text{head}(-e)) - f(\text{tail}(-e))\bigr) =
\pi_{F^0 \to F^1}\bigl(f(\text{tail}(e)) - f(\text{head}(e))
\bigr) = -(d_n f)(e)$, using $\R$-linearity of $\pi_{F^0 \to F^1}$.
Hence $d_n f \in X_n^{1, \bullet}$ (which incorporates the
antisymmetry convention of
\cite[Definition~\texttt{def:Xn-1}]{RefinementSpaces}).
\end{definition}
...
\begin{proof}
Write $\pi := \pi_{F^0 \to F^1}$, $\iota := \iota_{F^1 \to F^0}$,
and note that with respect to the orthogonal-direct-sum inner
product on $F^0 = V_{H_4} \oplus V_{D_4} \oplus V_{16} \oplus V_8$
and the standard real inner product on $F^1 = \Im(\OO) \subset
V_8$, the projection-inclusion pair is dual:
$\pi^* = \iota$ in
$\langle \pi(x), y \rangle_{F^1}
= \langle x, \iota(y) \rangle_{F^0}$
for $x \in F^0$, $y \in F^1$.

For $f \in X_n^{0, \bullet}$, $A \in X_n^{1, \bullet}$, by
\cite[Definition~\texttt{def:Xn-1}]{RefinementSpaces} the $X_n^1$
inner product carries a factor $\tfrac{1}{2}$ to compensate for
the antisymmetric oriented-edge double count. Compute
\begin{align*}
\langle d_n f, A \rangle_{X_n^1}
&= \tfrac{1}{2} \sum_{e \in E^{\rm or}(G_n^\bullet)}
   \bigl\langle \pi\bigl(f(\text{head}(e)) - f(\text{tail}(e))\bigr),
   A(e) \bigr\rangle_{F^1} \\
&= \tfrac{1}{2} \sum_{e}
   \bigl\langle f(\text{head}(e)) - f(\text{tail}(e)),
   \iota(A(e)) \bigr\rangle_{F^0}
   \qquad (\text{using } \pi^* = \iota) \\
&= \tfrac{1}{2} \sum_v \Bigl\langle f(v),
     \sum_{e:\, v=\text{head}(e)} \iota(A(e))
     - \sum_{e:\, v=\text{tail}(e)} \iota(A(e))
   \Bigr\rangle_{F^0}
   \qquad (\text{regroup over vertices}) \\
&= \sum_v \bigl\langle f(v), (d_n^* A)(v) \bigr\rangle_{F^0}
   = \langle f, d_n^* A \rangle_{X_n^0},
\end{align*}
using Definition~\ref{def:coboundary-adjoint} for the last line
and the standard discrete-Hodge regrouping
\cite[\S 1.2]{Chung}; the antisymmetry of $A$ on each pair
$(e, -e)$ ensures the regrouped vertex sum is well-defined.
\end{proof}
...
\[
  \|d_n f\|^2 \leq \sum_e (\|f(\text{head}(e))\|^2 +
    ...
    \|f\|^2,
\]
using that each vertex $v$ appears in at most
$2\deg(G_n^\bullet)$ oriented-edge incidences (each unoriented
edge incident to $v$ contributes one head-incidence and one
tail-incidence at $v$ across its two orientations); the
displayed factor $2\deg(G_n^\bullet)$ in the second inequality
absorbs both head and tail sums. Hence $\|d_n\|_{\op} \leq \sqrt{2
...
\begin{definition}[Curvature, off-diagonal coboundary, mass
quadratic forms]
\label{def:RnEnQn}
...
\begin{align*}
  ...
  E_n(\Phi) &:= \tfrac{1}{2} \langle \Phi, B_n \Phi \rangle_{X_n}
    \quad \text{(off-diagonal coboundary form)}, \\
...
$\pm\sqrt{\lambda}$ for each $\lambda > 0$ in
$\spec(d_n^* d_n)$, plus zero with appropriate multiplicity).
The form $E_n$ here is the off-diagonal coboundary form; this
is \emph{not} the positive kinetic square that earlier
correspondence-foundations drafts called ``kinetic.'' Downstream
papers citing kinetic energy should distinguish whether they
mean the indefinite off-diagonal form $E_n$ defined here or the
positive square $\|d_n \Phi^0\|^2 + \|d_n^* \Phi^1\|^2$.
\end{definition}
...
\[
  \|G_{n, \eta}|_{Y_n}\|_{\op} \leq \max(|1 - \eta m_n|, |1 - \eta
    M_n|) < 1.
\]
The displayed worst-case bound is minimised over $\eta$ at
$\eta^* = 2/(m_n + M_n)$, giving the upper bound
$\|G_{n, \eta^*}|_{Y_n}\|_{\op} \leq (M_n - m_n)/(M_n + m_n)
=: \rho_n < 1$. (Equality of $\|G_{n, \eta^*}|_{Y_n}\|_{\op}$
to $\rho_n$ holds when $m_n = \min \mathrm{spec}(L_n|_{Y_n})$ and
$M_n = \max \mathrm{spec}(L_n|_{Y_n})$, but not in general.)
\end{theorem}

\begin{proof}
On $Y_n$, $L_n$ is self-adjoint with spectrum contained in the
interval $[m_n, M_n]$ by Definition~\ref{def:coercive}; hence
$I - \eta L_n|_{Y_n}$ has spectrum contained in
$[1 - \eta M_n, 1 - \eta m_n]$. For $0 < \eta < 2/M_n$,
$|1 - \eta \lambda| < 1$ for all $\lambda \in [m_n, M_n]$
(since $0 < \eta < 2/\lambda$ for every $\lambda \geq m_n > 0$).
By the spectral theorem on a finite-dimensional Hilbert space,
$\|I - \eta L_n|_{Y_n}\|_{\op}$ is bounded above by
$\sup_{\lambda \in [m_n, M_n]} |1 - \eta \lambda|
= \max(|1 - \eta m_n|, |1 - \eta M_n|)$ (with equality if and
only if $m_n, M_n$ are spectral endpoints of $L_n|_{Y_n}$).
Minimising this upper bound over $\eta$, the two summands
$|1 - \eta m_n|$ and $|1 - \eta M_n|$ become equal in magnitude
and opposite in sign at $\eta^* = 2/(m_n + M_n)$, giving the
stated bound.
\end{proof}

\begin{proposition}[Negative spectrum obstructs global contraction]
...
\begin{proof}
Write $\pi := \pi_{F^0 \to F^1}$ for the canonical projection of
Definition~\ref{def:coboundary}.
For $f \in X_{n+1}^0$ and $e \in E^{\rm or}(G_n^\bullet)$:
\[
  (d_n (p_{n+1, n}^0 f))(e)
  = \pi\bigl((p_{n+1, n}^0 f)(\text{head}(e)) - (p_{n+1, n}^0 f)
    (\text{tail}(e))\bigr)
  = \pi\bigl(f(\text{head}(e)) - f(\text{tail}(e))\bigr)
\]
...
\begin{align*}
  (d_{n+1} f)(e_1) &= \pi\bigl(f(m) - f(\text{tail}(e))\bigr), \\
  (d_{n+1} f)(e_2) &= \pi\bigl(f(\text{head}(e)) - f(m)\bigr).
\end{align*}
The parent-fibre average of $d_{n+1} f$ from
\cite[Definition~\texttt{def:p1} (using the edge-parent map of Definition~\texttt{def:edge-parent})]{RefinementSpaces}, plus
$\R$-linearity of $\pi$, gives
\[
  (p_{n+1, n}^1 d_{n+1} f)(e)
  = \tfrac{1}{2}\bigl((d_{n+1} f)(e_1) + (d_{n+1} f)(e_2)\bigr)
  = \tfrac{1}{2}\, \pi\bigl(f(\text{head}(e)) - f(\text{tail}(e))\bigr)
  = \tfrac{1}{2} (d_n p_{n+1, n}^0 f)(e),
\]
which proves the claimed factor-$\tfrac{1}{2}$ identity (both
sides $F^1$-valued).
\end{proof}

\begin{remark}[The factor-$\tfrac{1}{2}$ is structural]
\label{rmk:factor-structural}
With the present unscaled undivided-difference coboundary $d_n$
at every refinement level and the averaging bonding map
$p_{n+1, n}^1$ of \cite[Definition~\texttt{def:p1} (using the edge-parent map of Definition~\texttt{def:edge-parent})]{RefinementSpaces}, the factor
$\tfrac{1}{2}$ in Proposition~\ref{prop:coboundary-refinement} is
unavoidable: the averaging step in $p_{n+1, n}^1$ contributes
exactly the $\tfrac{1}{2}$ that appears on the right-hand side.
Alternative normalizations (e.g.\ scaling $d_n$ by $2^n$ or by an
edge-length factor) move the factor elsewhere in the identity
without removing it. We keep the factor of $\tfrac{1}{2}$
explicit and propagate it through the downstream operator
identities.
\end{remark}

\begin{proposition}[Mass-block refinement compatibility]
\label{prop:mass-refinement}
...
\item The mass block $C_n$ is refinement-compatible exactly:
  $p C_{n+1} = C_n p$ (Proposition~\ref{prop:mass-refinement}).
...
Proposition~\ref{prop:mass-refinement}, $p_{n+1, n} C_{n+1} =
...
\item exact compatibility for the mass block
  (Proposition~\ref{prop:mass-refinement});
...
The word ``intertwiner'' is used here for continuity with prior
literature; it does \emph{not} mean that the resulting operator
intertwines a fixed symmetry action. Equivariance is established
only conditionally in Theorem~\ref{thm:intertwiner-bounds}(4),
under the additional hypothesis that every generator factor
satisfies the prescribed intertwining relation. Without that
hypothesis, an admissible expression in the syntactic sense above
is just a bounded linear map between finite-dimensional Hilbert
spaces.
\end{definition}
...
$j_{n, n+1}^1$ (edge embedding) & $\|j^1\|_{\op} = 2^{-1/2}$ &
  ...
  the adjoint identity $\langle j^1 B, j^1 B \rangle =
  \langle B, p^1 j^1 B \rangle$ gives
  $\|j^1 B\|^2 = \tfrac{1}{2}\|B\|^2$ \\
...
\end{tabular}
\end{center}
The $G_2$-stabilizer fibre map of
\cite[Definitions~\texttt{def:G2}, \texttt{def:stab-u} and
Fact~\texttt{fact:SU3-stab}]{AlgebraicSubstrate} (an element
of the stabilizer of a unit imaginary octonion in $G_2 \subset
\mathrm{Aut}(\OO)$, identified with $\mathrm{SU}(3)$ acting on
the orthogonal complement of the chosen unit) acts on
$\Im(\OO) = F^1$ orthogonally with respect to the standard
octonion inner product $\langle a, b \rangle_\OO := \Re(a\bar b)$:
every octonion automorphism preserves the octonion norm
\cite[Theorem~6.6]{Schafer} (and hence its real polarization),
and restriction to a $G_2$-invariant subspace remains orthogonal.
Therefore the corresponding fibre operator has operator norm
exactly $1$ on $F^1$, and its $\ell^2$-extension to $X_n^1$ also
has operator norm exactly $1$ (apply the bound fibrewise on each
oriented edge).
\end{proposition}
...
orthogonal transformations). The $G_2$-stabilizer norm equality
is justified above the proof.
\end{proof}
...
map between its finite-dimensional domain and codomain.
Specifically:
\begin{enumerate}
...
\item Proposition~\ref{prop:coboundary-refinement}: the explicit
  ...
  \tfrac{1}{2} d_n p^0$. Proposition~\ref{prop:mass-refinement}:
...
(Theorem~\ref{thm:flow-exists}), continuous-time energy
dissipation $\frac{d}{dt} F_n(\Phi_n(t)) = -\|L_n \Phi_n(t)\|^2$
(Proposition~\ref{prop:energy-dissipation}) and one-step Euler
energy descent under a step-size condition
(Proposition~\ref{prop:euler-energy-decrease}), strict contraction
...
(Proposition~\ref{prop:mass-refinement}), with full
...
\item No convergence target. The only proved
  refinement-compatible flow is the mass-only case $L_n =
  -\gamma C_n$ (with $\alpha = \beta = 0$), whose flow is
  $e^{\gamma t C_n}$ and is generally norm-expanding rather
  than contracting; we do not assert any attracting state, fixed
  point, or continuum target for $\Phi_n(t)$ as $t \to \infty$
  or as $n \to \infty$.
\item No target-side metric / hydrodynamic / gauge / spectral /
...
$R_n, E_n, Q_n$ & curvature, off-diagonal coboundary, mass forms & Def.~\ref{def:RnEnQn} \\
...
Academic Press, 1966 (reprinted Dover, 1995). Octonion
automorphisms preserve the octonion norm: Theorem~6.6.
