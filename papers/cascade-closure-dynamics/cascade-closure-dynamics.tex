...
explicit finite-generator list of \emph{admissible finite-level
...
\item No universal ``all correspondences are Borel and
  ...
  Section~\ref{sec:intertwiners} gives a finite generator list
...
\item[\textbf{Internal cascade infrastructure.}]
  ...
  $j_{n, n+1}$, and the commuting Coxeter/$\sigma$ actions.
...
intertwining. Section~\ref{sec:intertwiners} replaces the false
global Borel/equivariance theorem by explicit finite-generator
...
$\ell^2$-spaces from \cite[Defs.~4.2, 4.3]{RefinementSpaces}, with
...
\cite[Prop.~4.4]{RefinementSpaces}:
...
\begin{proof}
Direct from \cite[Prop.~4.4]{RefinementSpaces}.
\end{proof}
...
endpoints (with orientation). Here $f(\text{head}(e)) -
f(\text{tail}(e))$ is computed in the fibre $F^0$ of
\cite[Notation~3.3]{RefinementSpaces}, and the difference is then
projected to $F^1 = \Im(\OO)$ via the canonical quotient $F^0
\to F^0 / (\text{non-}\Im(\OO) \text{ components}) \to \Im(\OO)$
(i.e., taking the $V_8 = \OO$ component, then projecting to its
imaginary part).
...
convention of \cite[Def.~4.3]{RefinementSpaces}).
\end{definition}
...
for $A \in X_n^{1, \bullet}$ and $v \in V(G_n^\bullet)$,
\[
  (d_n^* A)(v) = \tfrac{1}{2} \sum_{e : v = \text{head}(e)}
    A(e) - \tfrac{1}{2} \sum_{e : v = \text{tail}(e)} A(e),
\]
...
top-left block $d_n^* d_n$ is the classical combinatorial graph
Laplacian on $X_n^0$ (positive semi-definite by construction); the
bottom-right block $d_n d_n^*$ is the edge-Laplacian on $X_n^1$
...
subdivides edges and introduces new vertices with higher degree.
This is expected and does not affect the finite-level theorems of
this paper. For the infinite-level / inverse-limit theorems of
P3, the bounds remain well-defined because the inverse limit is
...
\label{def:Fn}
Fix real constants $\alpha, \beta, \gamma > 0$, to remain fixed
throughout the paper. The \emph{closure functional at level $n$}
is
\[
...
\begin{corollary}[Flow is reversible]
\label{cor:flow-reversible}
The flow $e^{-t L_n}$ extends to all $t \in \R$, forming a
one-parameter group of bounded operators. In particular, the flow
is invertible on $X_n$.
\end{corollary}
...
map of \cite[Thm.~5.3]{RefinementSpaces} (extended to the
...
We now need explicit verification that the constituent operators
$A_n, B_n, C_n$ form refinement-compatible families. This is a
level-by-level claim about the specific coboundary, Laplacian, and
projection operators of \S\ref{sec:operators}.
...
of \cite[Def.~5.2]{RefinementSpaces}, $p_{n+1, n}^0$ is the
restriction of \cite[Def.~5.1]{RefinementSpaces}, and the level-
$(n+1)$ edges subdivide the level-$n$ edges via the canonical
edge-parent map of \cite[Def.~5.2]{RefinementSpaces}.
\end{proposition}
...
$p^0$ = restriction from \cite[Def.~5.1]{RefinementSpaces}).

On the other side, the level-$(n+1)$ edge $e$ subdivides as $e_1,
e_2$ with $\text{head}(e_1) = \text{tail}(e_2) = m$ (the midpoint
of $e$), so
\begin{align*}
...
\cite[Def.~5.2]{RefinementSpaces} is
\[
...
$p_{n+1, n}^1$ of \cite[Def.~5.2]{RefinementSpaces}. Changing the
...
$n$ consists of the following bounded operators:
\begin{itemize}
...
\item flow operators: $e^{-t L_n}$ for $t \geq 0$;
\item fixed fibre maps from \cite{AlgebraicSubstrate}: the
  $\sigma$-involution on $V_{H_4, \Q(\varphi)}$, the icosian
  star-map $\ast$, and the stabilizer map $\Im(\OO) \to \Im(\OO)$
  fixing a chosen unit $i_0$.
\end{itemize}
\end{definition}
...
$j_{n, n+1}^0$ (vertex embedding) & $\|j^0\|_{\op} = 1$ &
  \cite[Prop.~5.5]{RefinementSpaces} (adjoint of restriction,
  isometric on image) \\
$j_{n, n+1}^1$ (edge embedding) & $\|j^1\|_{\op} = 2^{-1/2}$ &
  \cite[Prop.~5.5]{RefinementSpaces} (rescaled replication:
...
$L_n$ (gradient operator) & $\|L_n\|_{\op} \leq 4\alpha\deg +
  2\beta\sqrt{\deg} + \gamma$ & Thm.~\ref{thm:gradient-operator}(3) \\
$e^{-t L_n}$ (flow, $t \geq 0$) & $\|e^{-t L_n}\|_{\op} \leq
  e^{t \|L_n\|_{\op}}$ & Thm.~\ref{thm:flow-exists} \\
\hline
\end{tabular}
\end{center}
The fibre maps (icosian star, $\sigma$-involution, $G_2$
stabilizer) from P2 act on the individual fibres $F^0, F^1$; their
$\ell^2$-extension to $X_n^0, X_n^1$ has norm $\leq 1$ (each is
unitary or norm-reducing at the fibre level). Explicit references:
\cite[Def.~4.2]{AlgebraicSubstrate} for the icosian star,
\cite[Def.~7.4]{AlgebraicSubstrate} for the $G_2$ stabilizer.
\end{proposition}

\begin{proof}
Each row is verified by citation or by the referenced proposition.
$\|\rho_n(g)\|_{\op} = 1$ because $\rho_n(g)$ is unitary (it permutes
an $\ell^2$-orthonormal basis up to fibre-level unitaries). The
fibre-map claims use that the icosian star $\ast$ is an involution
of norm $1$ on $V_{H_4, \Q(\varphi)}$ and the $G_2$ stabilizer is
a compact group element of norm $1$ on $\Im(\OO)$.
\end{proof}
...
Specifically:
\begin{enumerate}
\item (Boundedness with printed bound.) If $T$ is expressed as
  a composition/sum of generators with the bounds of
  Proposition~\ref{prop:generator-bounds}, then $\|T\|_{\op}$ is
  bounded by the corresponding combination of the listed
  generator bounds (via $\|T_1 \pm T_2\|_{\op} \leq \|T_1\|_{\op} +
  \|T_2\|_{\op}$ and $\|T_1 T_2\|_{\op} \leq \|T_1\|_{\op} \|T_2\|_{\op}$).
...
\item (Symmetry intertwining, conditional.) If every generator
  $T_k$ appearing in the composition $T = T_{k_1} \circ \cdots
  \circ T_{k_m}$ satisfies a prescribed intertwining relation
  $\rho(g) T_k = T_k' \rho(g)$ with a symmetry $g$, then $T$
  inherits the same relation: $\rho(g) T = (T_{k_1}' \cdots
  T_{k_m}') \rho(g)$.
\end{enumerate}
\end{theorem}
...
\emph{(4) Symmetry intertwining.} By iterated application of the
generator intertwinings along the composition sequence.
\end{proof}
...
\item Definitions~\ref{def:generators}, \ref{def:admissible} and
  Theorem~\ref{thm:intertwiner-bounds}: finite generator list
...
intertwiner bounds from the finite generator list
(Theorem~\ref{thm:intertwiner-bounds}).''
...
\item No universal Borel/equivariance principle beyond the
  explicit finite generator list.
\end{enumerate}
...
(Paper P2.) Pinned results used here: Def.~4.2 (icosian star-map
on $\Lambda(E_8)$, identified with the coefficientwise $\sigma$ of
P1 via the remark ``$\ast$ is the coefficientwise $\sigma$ of P1''
in P2~\S 3.2); Def.~7.4 ($G_2$ stabilizer of a unit imaginary
octonion $= \mathrm{SU}(3)$); P2~\S 7 and Thm.~``$\sigma$-compatibility of
$\Q$-defined P2 maps'' (coefficientwise $\sigma$-compatibility,
including the fibre maps imported in
\S\ref{sec:intertwiners} of this paper).
...
(Paper P3.) Pinned results used here: Notation~3.3 (sector
fibres $F^0, F^1$); Def.~4.2, Def.~4.3 (finite-level $\ell^2$
spaces $X_n^0, X_n^1$); Prop.~4.4 (finite-level Hilbert structure);
Def.~5.1 (vertex bonding $p^0$, a.k.a.\ restriction);
Def.~5.2 (edge bonding $p^1$, a.k.a.\ parent-fibre averaging);
Thm.~5.3 (operator-norm contraction bounds for $p^0, p^1$);
Prop.~5.5 (embedding $j_{n, n+1} = (p_{n+1, n})^*$, giving
$\|j^0\|_{\op} = 1$ and $\|j^1 B\|^2 = \tfrac{1}{2}\|B\|^2$);
Def.~8.1 (branchwise Coxeter action $\rho_n(g)$, unitary on each
$X_n^{0, \bullet}, X_n^{1, \bullet}$).
