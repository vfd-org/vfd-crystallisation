...
level; (O3) closed for the scaled curvature operator
$\widetilde A_n = 2^n A_n^{\mathrm{vertex}}$ via an exact
Schur-complement identity; (O2) partially closed at the
boundary-energy / quadratic-form level; (O1) reduces to spectral
decomposition on the connected graph Laplacian. Strict operator-
and flow-level intertwining of (O2) for the curvature flow is the
only substantive open ask remaining. This makes the paper the programme's
...
\item In lieu of a worked occurrence, the paper supplies a
  ...
  \S\ref{ssec:worked-invocation}; (O1) and (O3) are open in the
  source state, and (O2) is closed only for a different operator
  ($L_n$, mass-only) than the one naturally used here ($A_n$).
...
Proposition~\texttt{prop:adjoint-refinement} for mass-block
...
to it. Note that
\cite{CascadeClosureDynamics} sets up its closure functional under the
parameter convention $\alpha, \beta, \gamma > 0$ globally, but the
imported \texttt{thm:flow-intertwining} is a sub-case theorem within
that source, restricted to the mass-only specialisation
$\alpha = \beta = 0$ (so that $L_n = -\gamma C_n$); this is the
...
\item \textbf{(O1)} reduces to a \emph{nontrivial}-selected-state
  ...
  $\phi^\star_N$ with $R_N(\phi^\star_N) = 0$ is precisely an element
  of $\ker A_N$, and in the finite-dimensional positive-semidefinite
  setting of \cite{CascadeClosureDynamics} convergence
  ...
  point. What is open in the source is therefore not generic
  convergence but rather the existence and selection of a
  \emph{nontrivial} top-rung fixed point, $\phi^\star_N \neq 0$,
  with a positive spectral gap of $A_N$ on the initial datum's
  $\ker A_N^\perp$ component. \textbf{Open-as-named-question} for
  nontrivial selected states; trivial in the zero-state reduction.
...
\item \textbf{(O3)} reduces to a quadratic-form inequality:
  ...
  theorem. \textbf{Open-as-named-question}, not closed in the
  source.
\end{itemize}
...
(\cite{CascadeRefinementCompat}~\S1.5), and partially
discharges (O2) at the boundary-energy / quadratic-form level (the
strict operator-level identity
$p^{0} \widetilde A_{n+1} = \widetilde A_n p^{0}$ remains false in
that abstract model, with explicit defect $D_n$ whose kernel
...
\cite{CascadeRefinementSpaces, CascadeClosureDynamics} requires the
named hypotheses \textup{(L1)}--\textup{(L3)} of
\cite{CascadeRefinementCompat}~\S1.5 and is not yet attempted. (O1)
reduces to spectral decomposition for the connected graph
Laplacian, with the trivial state always residual-zero and
nontrivial selection out of scope per
\S\ref{sec:aria}--\S\ref{sec:witness}. Strict flow-level
...
& (O0) closed at the finite-dimensional level; (O3) closed for the scaled curvature $\widetilde A_n = 2^n A_n^{\mathrm{vertex}}$ via the Schur-complement identity $\mathrm{Schur}(\widetilde A_{n+1}) = \widetilde A_n$ in the abstract scalar pure-midpoint refinement model of \cite{CascadeRefinementCompat}~\S1.5 (P3/P4-tower lift conditional on the named hypotheses (L1)--(L3) of \cite{CascadeRefinementCompat}~\S1.5); (O2) partially closed in the same abstract model at the boundary-energy / Schur level (strict operator/flow-level identity $p^{0} \widetilde A_{n+1} = \widetilde A_n p^{0}$ remains false, with explicit defect $D_n$ characterised via the unsigned vertex--edge incidence operator); (O1) reduces to spectral decomposition on the connected graph Laplacian. \\
...
mechanism specifies the conditions under which residual zero,
...
spaces plus closure dynamics), where (O0) is satisfied at the
finite-dimensional level and none of (O1)--(O3) is closed in the
source state, with the minimal upstream upgrade named explicitly. A compressed nine-row programme-placement table
...
& \textsf{[Th-imp]}
& \cite{CascadeClosureDynamics}: \texttt{thm:gradient-operator}, \texttt{prop:energy-dissipation}, \texttt{thm:coercive-contraction}, \texttt{prop:adjoint-refinement}, \texttt{thm:flow-exists}, \texttt{thm:flow-intertwining} (mass-only $\alpha = \beta = 0$) \\
...
& \textsf{[Mod]}
& \cite{ObserverInstance}; \S\ref{sec:aria} \\
\bottomrule
\end{longtable}
...
substrate, observer instance) by supplying the named
