...
refinement spaces plus closure dynamics), where (O0) is satisfied at
the finite-dimensional level by the source's Hilbert structure, and
none of (O1)--(O3) is closed in the source state. This makes the paper the programme's
...
\noindent The honest current verdict, read off the source state of
\cite{CascadeRefinementSpaces, CascadeClosureDynamics}: in the
imported P3/P4 finite-level tower, (O0) is satisfied at the
finite-dimensional level by the source's $X_n$ Hilbert structure
(standing analytic preconditions, $C^1$ bonding maps); among
(O1)--(O3), none is closed at
the level of generality the present paper requires, although (O2) is
closed for a different operator ($L_n$, mass-only) than the one
naturally used here ($A_n$). The minimal upstream upgrade that
would discharge the consumer API for the P3/P4 tower is therefore a
single source-side result on $A_n$: refinement-compatibility
$p_{n+1, n} A_{n+1} = A_n p_{n+1, n}$ together with the
quadratic-form inequality of (O3) and a kernel-existence /
spectral-gap result for $A_N$. The role of the present paper is to
make these three asks visible as the programme's actual dependency
gate, rather than to discharge them.
...
& (O1) and (O3) open in the source state; (O2) closed only for $L_n$ (mass-only), not for $A_n$. The minimal upstream upgrade required to discharge the consumer API for the P3/P4 tower is named there. \\
