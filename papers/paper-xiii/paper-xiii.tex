...
\end{equation}
where $f$ is a monotone function (e.g.\ $f(r) = e^{-r^2/\sigma^2}$ for a localised interaction, or $f(r) = r^2$ for a long-range harmonic coupling). States that are closer in the ambient space interact more strongly. The sign and shape of $f$ determine whether proximity acts effectively as an attractive or repulsive contribution within the total interaction landscape; the present paper uses it only as a generic structural coupling term.
...
\end{equation}
where $\Xi_{\mathrm{joint}}$ is the gap penalty of the union of supports (Paper~VIII~\citep{SmartVIII}). Connected composite configurations have $\Xi_{\mathrm{joint}} = 0$, while disconnected unions have $\Xi_{\mathrm{joint}} > 0$. Through the positive weight $\lambda_4$, disconnected joint configurations therefore raise the total closure cost, while connected composites are favoured relative to them.
