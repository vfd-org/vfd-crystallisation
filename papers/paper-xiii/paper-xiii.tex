...
The present paper introduces a structural interaction layer within the closure framework. It does not yet provide a quantum scattering theory; rather, it defines how multiple admissible states jointly modify the closure landscape and thereby influence one another's evolution.
...
\end{equation}
where $\lambda_i > 0$ are structural weights, not derived in this paper.
...
\end{equation}
where $f$ is a monotone function (e.g.\ $f(r) = e^{-r^2/\sigma^2}$ for a localised interaction, or $f(r) = r^2$ for a long-range harmonic coupling). States that are closer in the ambient space interact more strongly.
...
The binding term rewards jointly connected configurations:
\begin{equation}\label{eq:bind}
    I_{\mathrm{bind}}(\Psi) = -\mu\, \Xi_{\mathrm{joint}}(\Psi)
\end{equation}
where $\Xi_{\mathrm{joint}}$ is the gap penalty of the union of supports (Paper~VIII~\citep{SmartVIII}), and $\mu > 0$ controls binding strength. The negative sign means that jointly connected configurations \textit{lower} the total closure cost, favouring composite formation.
...
The first term drives $\psi_i$ toward its own closure; the second term encodes the influence of all other states on $\psi_i$'s evolution. Without the interaction term, the evolution is decoupled and reduces to $n$ independent copies of Paper~XII dynamics.
...
When this condition holds, the interaction term $\Fint$ lowers the total closure cost, creating a joint attractor basin not present in the uncoupled system. The bound configuration is more closure-compatible than the separated states: it is a composite that is ``cheaper'' to close than its parts.

\begin{remark}
This is the framework's structural analogue of binding energy: the difference $\sum\Fcal(\psi_i) - \Ftot(\Psi^*)$ measures how much the interaction lowers the total closure cost. In nuclear physics, the analogous quantity is the mass defect.
\end{remark}
...
Consider a state $\psi_1$ with disconnected support $\{2,4\}$ (quark-like, $\Xi > 0$) and a state $\psi_2$ with support $\{3\}$. Individually, $\psi_1$ is not closure-stable. Under the coupled dynamics with the binding term active, the joint configuration $\Psi = (\psi_1, \psi_2)$ can evolve toward a connected composite on $\{2,3,4\}$ with $\Ftot(\Psi^*) < \Fcal(\psi_1) + \Fcal(\psi_2)$. This models the formation of a baryon-like bound state from disconnected components.
\end{example}
...
Binding energy & $\sum\Fcal(\psi_i) - \Ftot(\Psi^*)$ \\
...
Interaction is interpreted as deformation of the closure landscape: the presence of one state modifies the effective basin structure experienced by another. Binding corresponds to the formation of joint attractors whose closure cost is lower than the separated configuration. Scattering-like processes are basin-crossing or basin-preserving transitions within the coupled dynamics.

This is not yet a scattering theory, a quantum field theory, or a dynamical model of gauge-boson exchange. It is the first structural interaction layer within the closure framework: the minimal extension that allows multiple states to influence one another's evolution. From this layer, extensions toward quantised dynamics, scattering amplitudes, and gauge coupling could in principle be pursued.
