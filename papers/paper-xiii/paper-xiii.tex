...
Equivalently, for each component $\psi_i$, the effective single-state functional becomes:
\begin{equation}
    \Fcal^{\mathrm{eff}}(\psi_i; \Psi) = \Fcal(\psi_i) + \nabla_{\psi_i}\Fint(\Psi)
\end{equation}
so that interaction can be viewed as a state-dependent deformation of the intrinsic closure landscape.
