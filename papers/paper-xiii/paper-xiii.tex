...
\end{equation}
where $f$ is a monotone function (e.g.\ $f(r) = e^{-r^2/\sigma^2}$ for a localised interaction, or $f(r) = r^2$ for a long-range harmonic coupling). States that are closer in the ambient space interact more strongly. The sign convention and shape of $f$ determine whether proximity contributes effectively as attraction or repulsion within the total interaction landscape; the present paper leaves this choice at the level of structural ansatz and uses the term only as a generic coupling component.
